...
function itsp_user_search()
{
    ...
    /* itsp module fields for possible searching and identification */
    ...
    /* itsp for search where an author is involved, not included in this itsp module */
    ...
    */
    ...
    /* ITSP code that you might use to find the uid of the author we're looking for
     * Adjust for your own module if required. Demonstrates use of roles api call
     * rather than direct db function to protect against roles table changes for itsp
     */
    ...
    */
    ...
    /* Call search hook for itsp module to Search itsp information */
    if (!empty($q) || !empty($search)) {
        $data['itsp'] = xarModAPIFunc('itsp','user','search',$search);
    }
    /* Prepare the message to return to search template if no match found */
    if (empty($data['itsp'])){
        $data['status'] = xarML('No ITSP item found that matches your search');
    }
