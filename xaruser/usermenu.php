 ...
 *
 */
function accessmethods_user_usermenu($args)
{
    extract($args);
    ...
    switch(strtolower($phase)) {
        case 'menu':
            ...
            $data = xarTplModule('accessmethods','user', 'usermenu_icon', array('iconbasic'    => $icon));
        ...
        case 'form':
            ...
            // vars off site.
            $authid = xarSecGenAuthKey('accessmethods');

            $submitlabel = xarML('Submit');
            // Now lets send the data to the template which name we choose here.
            $data = xarTplModule('accessmethods','user', 'usermenu_form', array('authid'      => $authid,
