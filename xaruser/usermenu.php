...
function maxercalls_user_usermenu($args)
{
    ...
    switch (strtolower($phase)) {
        case 'menu':
            ...
            $icon = 'modules/maxercalls/xarimages/preferences.gif';
            // Now lets send the data to the template which name we choose here.
            $data = xarTplModule('maxercalls', 'user', 'usermenu_icon', array('iconbasic' => $icon));
        ...
        case 'form':
            ...
            // vars off site.
            $authid = xarSecGenAuthKey('maxercalls');
            ...
            $value = xarModGetUserVar('maxercalls', 'itemsperpage', $uid);
            // if (empty($value)){
            // $value = xarModGetVar('maxercalls', 'itemsperpage');
            // }
            // Now lets send the data to the template which name we choose here.
            $data = xarTplModule('maxercalls', 'user', 'usermenu_form', array('authid' => $authid,
                    'name' => $name,
                    'uid' => $uid,
                    'value' => $value));
            break;
        ...
        case 'update':
            ...
            // The maxercalls module is not setting any user vars at this time, but an maxercalls
            ...
            xarModSetUserVar('maxercalls', 'itemsperpage', $itemsperpage, $uid);
