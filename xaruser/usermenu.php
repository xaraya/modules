 ...
 */
function phplot_user_usermenu($args)
{
    extract($args);
    ...
    switch (strtolower($phase)) {
        case 'menu':
            ...
            $icon = 'modules/phplot/xarimages/preferences.gif';

            /* Now lets send the data to the template which name we choose here. */
            $data = xarTplModule('phplot', 'user', 'usermenu_icon', array('iconbasic' => $icon));
        ...
        case 'form':
             ...
             * vars off site.
             */
            $authid = xarSecGenAuthKey('phplot');
             ...
             */
            $value = xarModGetUserVar('phplot', 'itemsperpage', $uid);
            /* if (empty($value)){
             * $value = xarModGetVar('phplot', 'itemsperpage');
             * }
             * Now lets send the data to the template which name we choose here.
             */
            $data = xarTplModule('phplot', 'user', 'usermenu_form', array('authid' => $authid,
                    'name' => $name,
                    'uid' => $uid,
                    'value' => $value));
            break;
        ...
        case 'update':
             ...
             * The phplot module is not setting any user vars at this time, but an phplot
            ...
            xarModSetUserVar('phplot', 'itemsperpage', $itemsperpage, $uid);
