...
function uploads_user_download()
{
    ...
    if ( (!isset($fileId) || empty($fileId)) && (!isset($vpath) || empty($vpath))) {
        $msg = xarML('Missing both parameter [#(1)] and [#(2)] for function [#(3)] in module [#(4)]. One or the other is required for successful operation.',
                     'fileId','vpath', 'db_get_file_data','uploads');
        xarErrorSet(XAR_SYSTEM_EXCEPTION, 'BAD_PARAM', new SystemException($msg));
        return FALSE;
    }

    if (!isset($fileId)) {
        $vpathInfo = xarModAPIFunc('uploads', 'vdir', 'path_decode', array('path' => $vpath));
    ...
    if (xarSecurityCheck('ViewUploads', 1, 'File', $instance)) {
        ...
        if ($fileInfo['storetype']['value'] & _UPLOADS_STORE_FILESYSTEM || ($fileInfo['storetype']['value'] == _UPLOADS_STORE_DB_ENTRY)) {
        ...
        } elseif ($fileInfo['storetype']['value'] & _UPLOADS_STORE_DB_FULL) {
        ...
        $result = xarModAPIFunc('uploads', 'fs', 'push', $fileInfo);
