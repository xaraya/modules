 ...
 */
    function payments_user_amount()
    {
        ...
        if (!xarVarFetch('allowEdit_Payment', 'int', $data['allowEdit_Payment'], 0, XARVAR_DONT_SET)) {
            return;
        }
        ...
        if (!xarVarFetch('errorFlag', 'int:0:', $errorFlag, null, XARVAR_DONT_SET)) {
            return;
        }
        if (!$errorFlag) {
        ...
        if (!xarVarFetch('b', 'str:', $gestpaydata, null, XARVAR_DONT_SET)) {
            return;
        }
        $data['authid'] = xarSecGenAuthKey();
        //Psspl:check if user selects 'MakeChanges'.
        $data['MakeChanges'] = null;
        if (!xarVarFetch('MakeChanges', 'int:0:', $MakeChanges, null, XARVAR_DONT_SET)) {
            return;
        }
