...
function authentication_user_login()
{
    ...
    $unlockTime  = (int) xarSessionGetVar('authentication.login.lockedout');
    $lockouttime=xarModGetVar('authentication','lockouttime')? xarModGetVar('authentication','lockouttime') : 15;
    $lockouttries =xarModGetVar('authentication','lockouttries') ? xarModGetVar('authentication','lockouttries') : 3;

    if ((time() < $unlockTime) && (xarModGetVar('authentication','uselockout')==true)) {
    ...
    switch(strtolower($state)) {
        ...
        case ROLES_STATE_NOTVALIDATED:
            ...
            xarResponseRedirect(xarModURL('authentication', 'user', 'getvalidation'));
        ...
        default:
            ...
            // Log the user in
            $res = xarModAPIFunc('authentication','user','login',array('uname' => $uname, 'pass' => $pass, 'rememberme' => $rememberme));
            if ($res === NULL) return;
            elseif ($res == false) {
                // Problem logging in
                ...
                $attempts = (int) xarSessionGetVar('authentication.login.attempts');

                if (($attempts >= $lockouttries) && (xarModGetVar('authentication','uselockout')==true)){
                    // set the time for fifteen minutes from now
                    xarSessionSetVar('authentication.login.lockedout', time() + (60 * $lockouttime));
                    xarSessionSetVar('authentication.login.attempts', 0);
                    $msg = xarML('Problem logging in: Invalid username or password.  Your account has been locked for #(1) minutes.', $lockouttime);
                    xarErrorSet(XAR_USER_EXCEPTION, 'LOGIN_ERROR', new DefaultUserException($msg));
                    return;
                } else{
                    $newattempts = $attempts + 1;
                    xarSessionSetVar('authentication.login.attempts', $newattempts);
