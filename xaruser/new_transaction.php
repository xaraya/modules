...
function payments_user_new_transaction()
{
    ...
    if (!xarVar::fetch('type_changed',  'int',    $type_changed,          0,     xarVar::NOT_REQUIRED)) return;
    if (!xarVar::fetch('debit_account', 'int',    $data['debit_account'], 0,     xarVar::NOT_REQUIRED)) return;
...
#
    if (!xarVar::fetch('api',          'str',    $api,            '', xarVar::NOT_REQUIRED)) return;
...
#
    if (!empty($info['payment_object']) && !empty($info['payment_itemid'])) {
        ...
        if (count($items) == 1) {
            $item = current($items);
            xarController::redirect(xarController::URL('payments', 'user', 'modify_transaction', array('itemid' => $item['id'], 'api' => $api)));
            return true;
        }
    }
...
#
    ...
    if (!xarVar::fetch('payment_type',  'str',    $data['payment_type'],  '1',   xarVar::NOT_REQUIRED)) return;
