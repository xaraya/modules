...
function messages_user_new() 
{
    ...
    if ($send || $draft || $saveandedit) {
        ...
        if ($saveandedit) {
            xarController::redirect(xarModURL('messages','user','modify', array('id' => $id)));
            return true;
        }
        ...
        if ($redirect == 'new') {
            xarController::redirect(xarModURL('messages','user','new'));
        } else {
            xarController::redirect(xarModURL('messages','user','view', array('folder' => $redirect)));
        }
        return true;
    }
