...
function reminders_user_send_lookup_email($args)
{
    // Xaraya security
    ...
    if (!xarVarFetch('participant_id',    'int', $data['participant_id'], 0, XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('course_id',         'int', $data['course_id'],      0, XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('message_id',        'int', $data['message_id'],     0, XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('message_body',      'str', $data['message_body'],   '', XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('subject',           'str', $data['subject'],        '', XARVAR_NOT_REQUIRED)) return;
	if (!xarVarFetch('trusted_attachment','str', $data['trusted_attachment'],     '', XARVAR_NOT_REQUIRED)) return;
