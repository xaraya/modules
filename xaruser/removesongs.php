 ...
 */
function mp3jukebox_user_removesongs($args)
{
     ...
     */
    if (!xarVarFetch('playlistid',   'id', $playlistid,   $playlistid)) return;
    if (!xarVarFetch('removesong',     'array', $removesong,     $removesong)) return;

    // Argument check
    $invalid = array();
    if (empty($playlistid) || !is_int($playlistid)) {
        $invalid['playlistid'] = 1;
    }
    if (!is_array($removesong)) {
        $invalid['removesong'] = 1;
    }
    ...
    if (count($invalid) > 0) {
         ...
         */
        return xarModFunc('mp3jukebox', 'user', 'display',
     ...
     */
    $playlist = xarModAPIFunc('mp3jukebox',
                          'user',
                          'get',
                          array('playlistid' => $playlistid)
                      );
     ...
     */
    if (!isset($playlist) && xarCurrentErrorType() != XAR_NO_EXCEPTION) return; // throw back
     ...
     */
     if (!xarSecurityCheck('EditMP3Jukebox', 0, 'Playlist', "$playlist[title]:All:$playlist[playlistid]")) {
        $msg = xarML('Not allowed to make changes to playlist');
        xarErrorSet(XAR_USER_EXCEPTION, 'NOT_ALLOWED',
            new SystemException($msg));
        return;
    }

    $remove = xarModAPIFunc('mp3jukebox','user','removesongs', array('playlistid' => $playlistid, 'removesong' => $removesong));

    xarResponseRedirect(xarModURL('mp3jukebox', 'user', 'display',array('playlistid' => $playlistid)));
