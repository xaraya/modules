...
function products_user_default()
{
    $main_content = '';
    ...
    $currentlang = xarModAPIFunc('products','user','get_language',array('locale' => $data['language']));
    ...
    $category_depth = 'top';
    if (isset($cPath)) {
        $q = new xenQuery('SELECT',$xartables['product_product_to_categories']);
    ...
    if (xarModAPIFunc('products','user','check_categories_status', array('categories_id' =>$current_category_id)) >= 1) {
...
//        include(DIR_WS_MODULES . FILENAME_ERROR_HANDLER);
        ...
        //    }
            ...
            foreach ($q->output() as $categories) {
                $rows++;
//                $cPath_new = xarModAPIFunc('products','user','get_path', array('nodeid' => $categories['cid']));
                ...
                $categories_content[]=array(
                      ...
                      'categories_link' => xarModURL('products','user','default', array('cPath' => $cPath_new)),
...
//            echo var_dump($category);exit;
            ...
            if ($category['categories_template']=='' || $category['categories_template']=='default') {
                $files=array();
                if ($dir= opendir('modules/products/xartemplates/category_listing/')){
...
//            echo var_dump($data);exit;
            return xarTplModule('products','user', $category['categories_template'],$data);
...
*/
    ...
    //STOPPED HERE
                ...
                //    }
                    ...
                    $q->addtable($xartables['product_specials'],'s');
...
//need to add fsk18 and sorting
            ...
            if ($manufacturers_id != 0) {
                $q = new xenQuery('SELECT', $xartables['product_manufacturers'], 'manufacturers_image');
