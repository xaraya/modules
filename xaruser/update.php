 ...
 */
/**
 * update item from articles_user_modify
 *
...
function articles_user_update()
{
    ...
    if (empty($ptid) || !isset($pubtypes[$ptid])) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                     'publication type', 'user', 'update', 'Articles');
        xarErrorSet(XAR_SYSTEM_EXCEPTION, 'BAD_PARAM',
                       new SystemException($msg));
        return;
    }
...
// TODO: make $status dependent on permissions ?
    ...
    if ($preview || count($invalid) > 0) {
        $data = xarModFunc('articles','user','modify',
                             array('preview' => true,
                                   'article' => $article,
        ...
        unset($article);
        if (is_array($data)) {
            return xarTplModule('articles','user','modify',$data);
        } else {
            return $data;
        }
    }
    ...
    // Pass to API
    if (!xarModAPIFunc('articles', 'user', 'update', $article)) {
        return;
    }
    unset($article);
    ...
    if (isset($save) && xarSecurityCheck('EditArticles',0,'Article',$ptid.':All:All:All')) {
        xarResponseRedirect(xarModURL('articles', 'user', 'modify',
                                      array('aid' => $aid)));
        return true;
    }
    ...
    // Return to the original user view
    ...
    // if we can edit articles, go to user list view, otherwise go to user view
    if (xarSecurityCheck('EditArticles',0,'Article',$ptid.':All:All:All')) {
        xarResponseRedirect(xarModURL('articles', 'user', 'viewlist',
                                      array('ptid' => $ptid)));
    } else {
        xarResponseRedirect(xarModURL('articles', 'user', 'view',
                                      array('ptid' => $ptid)));
