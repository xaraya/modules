...
function crispbb_user_forum_index()
{
    ...
    if (!xarVar::fetch('action', 'enum:read', $action, '', xarVar::NOT_REQUIRED)) {
        return;
    }
    if ($action == 'read') {
        if (!xarVar::fetch('fid', 'id', $readfid, null, xarVar::DONT_SET)) {
            return;
        }
    }
        ...
        foreach ($categories as $cid => $category) {
            ...
            if (!empty($numforums)) {
                foreach ($forums[$cid] as $fid => $forum) {
                    ...
                    if (!empty($seenLevel)) {
                        $seenLevels[$seenLevel] = $forum['fprivileges'][$seenLevel];
                    }
                    if (!empty($tracker)) {
