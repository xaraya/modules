...
/**
* File: $Id: addcat.php,v 1.5 2005/03/29 16:23:26 caseygeene Exp $
*
...
function julian_user_addcat()
{
   ...
   if (!strcmp($addaction,"Add")){
     ...
     xarResponseRedirect(xarModURL('julian', 'user', 'addcat'));
   }
   ...
   $js_colors='';
    while(!$result->EOF)
    {
      ...
      $catinfo = xarModAPIFunc('categories', 'user', 'getcatinfo', array('cid'=>$result->fields[0]));
      if (!empty($catinfo)) {
          // Store names/colors.
          $name = $catinfo['name'];
          $js_names .= "\"".$name."\",";
          $js_colors .= "\"".$result->fields[1]."\",";
          $categories[$result->fields[0]]['cat_name'] = $name;
          $categories[$result->fields[0]]['color'] = $result->fields[1];
      }
      else {
