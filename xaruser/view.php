...
function phplot_user_view()
{
    ...
    $items = xarModAPIFunc('phplot',
        ...
        array('startnum' => $startnum,
              'numitems' => xarModGetUserVar('phplot',
              'itemsperpage',
              $uid)));
    ...
    foreach ($items as $item) {
         ...
         */
        if (xarSecurityCheck('ReadPHPlot', 0, 'Item', "$item[name]:All:$item[exid]")) {
            $item['link'] = xarModURL('phplot',
                'user',
                'display',
    ...
    $data['pager'] = xarTplGetPager($startnum,
        ...
        xarModURL('phplot', 'user', 'view', array('startnum' => '%%')),
        xarModGetUserVar('phplot', 'itemsperpage', $uid));

    /* We are changing the name of the page to raise
     * better search engine compatibility.
     */
    xarTplSetPageTitle(xarVarPrepForDisplay(xarML('View PHPlots')));
