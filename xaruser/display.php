 ...
 */
function accessmethods_user_display($args)
{
    ...
    if (empty($itemid)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'item id', 'user', 'display', 'accessmethods');
...
/* end APPROACH # 4 : getting only the raw item values via API */
    ...
    $data['is_bold'] = xarModGetUserVar('accessmethods', 'bold');

    xarVarSetCached('Blocks.accessmethods', 'itemid', $itemid);

    // call the display hooks for this item
    $item = array();
    $item['module'] = 'accessmethods';
    $item['returnurl'] = xarModURL('accessmethods', 'user', 'display',
                                   array('itemid' => $itemid));
