...
function crispbb_user_newtopic($args)
{
    ...
    $data = xarMod::apiFunc('crispbb', 'user', 'getforum', ['fid' => $fid, 'privcheck' => true]);
    ...
    if (!empty($data['floodcontrol']) && empty($errorMsg)) {
        ...
        if (!empty($lastpost)) {
            ...
            if ($lastpost['ptime'] > $now-$data['floodcontrol']) {
                ...
                $errorMsg['return_url'] = xarController::URL('crispbb', 'user', 'view', ['fid' => $fid]);
    ...
    if (!xarVar::fetch('ttitle', 'str:1:100', $ttitle, '', xarVar::NOT_REQUIRED)) {
        return;
    }
    ...
    if (!xarVar::fetch('tstatus', 'int:0:10', $tstatus, 0, xarVar::NOT_REQUIRED)) {
        return;
    }
    if (!xarVar::fetch('ttype', 'int:0:10', $ttype, 0, xarVar::NOT_REQUIRED)) {
        return;
    }
    if (!xarVar::fetch('topicicon', 'str', $topicicon, null, xarVar::NOT_REQUIRED)) {
        return;
    }
    ...
    if (!xarVar::fetch('approvereplies', 'checkbox', $approvereplies, false, xarVar::NOT_REQUIRED)) {
        return;
    }
    ...
    if (!xarVar::fetch('modname', 'str:1', $modname, '', xarVar::NOT_REQUIRED)) {
        return;
    }
    if (!xarVar::fetch('itemtype', 'id', $itemtype, 0, xarVar::NOT_REQUIRED)) {
        return;
    }
    ...
    */
    ...
    if (!empty($modname)) {
        ...
        if (empty($modid)) {
            ...
            $vars = ['module name', 'user', 'newtopic', 'crispBB'];
            throw new BadParameterException($vars, $msg);
            return;
        }
        if (empty($itemid)) {
            $msg = 'Invalid #(1) for #(2) function #(3)() in module #(4)';
            $vars = ['item id', 'user', 'newtopic', 'crispBB'];
            throw new BadParameterException($vars, $msg);
            return;
        }
        ...
        if (empty($settings['fid']) || $settings['fid'] != $fid) {
            $msg = 'Invalid #(1) for #(2) function #(3)() in module #(4)';
            $vars = ['fid', 'user', 'newtopic', 'crispBB'];
            throw new BadParameterException($vars, $msg);
            return;
        }
        $itemlinks = xarMod::apiFunc($modname, 'user', 'getitemlinks', ['itemids' => [$itemid], ], 0);
        if (!empty($itemlinks[$itemid])) {
            ...
            $ttitle .= ' ' . $itemid;
            $linkurl = xarController::URL($modname, 'user', 'display', ['itemtype' => $itemtype, 'itemid' => $itemid]);
        }
        ...
        if (empty($invalid) && !$preview) {
            ...
            if (!empty($tracker)) {
                ...
                $topicssince = xarMod::apiFunc(
                    ...
                    'user',
                    'gettopics',
                    ['fid' => $fid, 'starttime' => $lastreadforum]
                );
