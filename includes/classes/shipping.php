<?php
/* -----------------------------------------------------------------------------------------
   $Id: shipping.php,v 1.2 2003/11/05 16:35:16 fanta2k Exp $
  ...
  class shipping {
    ...
    function shipping($module = '') {
      ...
      if (defined('MODULE_SHIPPING_INSTALLED') && xarModAPIFunc('commerce','user','not_null',array('arg' => MODULE_SHIPPING_INSTALLED))) {
        $this->modules = explode(';', MODULE_SHIPPING_INSTALLED);

        $include_modules = array();

        if ( (xarModAPIFunc('commerce','user','not_null',array('arg' => $module))) && (in_array(substr($module['id'], 0, strpos($module['id'], '_')) . '.' . substr($PHP_SELF, (strrpos($PHP_SELF, '.')+1)), $this->modules)) ) {
    ...
    function quote($method = '', $module = '') {
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if (xarModAPIFunc('commerce','user','not_null',array('arg' => $module))) {
