 ...
 **/
    function httpClient($host = '', $port = '') {
      if (xarModAPIFunc('commerce','user','not_null',array('arg' => $host))) {
 ...
 **/
    function Connect($host, $port = '') {
      ...
      $this->url['host'] = $host;
      if (xarModAPIFunc('commerce','user','not_null',array('arg' => $port))) $this->url['port'] = $port;
...
// connect if necessary
      if ( ($this->socket == false) || (feof($this->socket)) ) {
        ...
        if (!xarModAPIFunc('commerce','user','not_null',array('arg' => $port))) $port = 80;

        if (!$this->socket = fsockopen($host, $port, $this->reply, $this->replyString)) {
          return false;
        }

        if (xarModAPIFunc('commerce','user','not_null',array('arg' => $this->requestBody))) {
