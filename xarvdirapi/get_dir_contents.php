...
function filemanager_vdirapi_get_dir_contents( $args )
{
    ...
    if (!isset($vdir_id) || empty($vdir_id)) {
        $msg = xarML('Missing parameter [#(1)] for function [(#(2)] in module [#(3)]',
                     'vdir_id', 'vdir_get_dir_contentents', 'filemanager');
    ...
    $vpath = xarModAPIFunc('filemanager', 'vdir', 'path_encode', array('vdir_id' => $vdir_id));

    $files = xarModAPIFunc('filemanager', 'user', 'db_get_file_entry', 
    ...
    if (count($files)) {
        foreach ($files as $key => $file) {
            ...
            $list[$key]['link']       = xarModURL('filemanager', 'user', $linkfunc, $linkargs);
            $list[$key]['comment']    = '';
