...
/**
   ...
   @return true of success
           false on failure
*/
function mailbag_admin_delete($args)
{
    // Get Vars
    ...
    if (empty($itemtype)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'item type', 'admin', 'delete', 'mailbag');
        xarExceptionSet(XAR_USER_EXCEPTION, 'BAD_PARAM',
                       new SystemException($msg));
        return $msg;
    }
    ...
    // do the check
    if (!xarSecurityCheck('deletemailbag',1,'item',$itemid)) return;
    ...
    xarResponseRedirect(xarModURL('mailbag', 'admin', 'view', array('itemtype' => $itemtype)));
