  ...
  if (!is_array($paper)){
    switch (strtolower($paper)){
      case 'letter':
        ...
        break;
      case 'a3':
        $size = array(0,0,841.9,1190.6);
        break;
      case 'a4':
      default:
        $size = array(0,0,595.3,841.9);
        break;
    }
...
function ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$gap,$size,&$y,$options=array(),$test=0){
  // uses ezText to add the text, and returns the height taken by the largest heading
  // if $test=1 then just run through the motions and determine if this heading is going to fit on the page
  // return true if it goes onto a new page
  ...
  foreach($cols as $colName=>$colHeading){
    ...
    $newPage1 = $this->ezText($colHeading,$size,array('aleft'=> $pos[$colName],'aright'=>($maxWidth[$colName]+$pos[$colName]),'justification'=>$justification),$test);
    if ($test && $newPage1){
      $newPage=$newPage1;
    }
    $dy=$y-$this->y;
...
//  $y0=$y+$height+$decender;
  ...
  if ($options['showHeadings']){
    // need to figure out just how much height this heading will take, and if it will not fit onto the space on the page
    // then put it onto a new page.
    $newPage=$this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$options['gap'],$options['fontSize'],$y,$options['cols'],1);
    if ($newPage){
      $this->ezNewPage();
      $y=$this->y;
      $y0=$y+$decender;
...
//        $y2=$y-$mx+3*$height+$decender-$height*$newPage;
