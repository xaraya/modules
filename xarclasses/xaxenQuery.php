<?php
  /**************************************************************************\
  * xaxenQuery class for SQL abstraction                                       *
...
// Constructor
//---------------------------------------------------------
    function xaxenQuery($type='SELECT',$tables='',$fields='')
    {
...
//            $this->rows = $result->_numOfRows;
        ...
        if ($display == 1) {
            if ($statement == '') {
                if ($this->fields == array() && $this->rowfields > 0) {
                ...
                while (!$result->EOF) {
                    for ($i=0;$i<$this->rowfields;$i++) {
                        if(!empty($this->fields[$i]['alias']))
                            $line[$this->fields[$i]['alias']] = $result->fields[$i];
                        elseif(!empty($this->fields[$i]['name']))
...
*/
    ...
    function _getcondition($key)
    {
        ...
        if (gettype($condition['field2']) == 'string' && !eregi('JOIN', $condition['op'])) {
            ...
            if ($this->usebinding && $condition['op'] != 'JOIN') {
                $this->bindvars[] = $condition['field2'];
                $sqlfield = '?';
            } else {
                $sqlfield = $condition['field2'];
        ...
        if ($condition['op'] == 'in') {
            if (is_array($sqlfield)) {
                $elements = array();
                if ($this->usebinding) {
                    foreach ($sqlfield as $element) {
                        $this->bindvars[] = $element;
                        $elements[] = '?';
                    }
                } else {
                    foreach ($sqlfield as $element) $elements[] = $this->dbconn->qstr($element);
                }
...
//        echo var_dump($this->tablelinks);exit;
...
*/
    ...
    function getrows()
    {
        ...
        if ($this->type == 'SELECT' && $this->rowstodo != 0 && $this->limits == 1) {
            if (!isset($this->dbconn)) $this->dbconn =& xarDBGetConn();
            if ($this->israwstatement || $this->uniqueselect) {
                $temp1 = $this->rowstodo;
                $temp2 = $this->startat;
                $this->rowstodo = 0;
                $this->startat = 0;
