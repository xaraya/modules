<?php
/**
 * Search the database for mp3jukebox items
 *
 * @package modules
 ...
 */
function mp3jukebox_userapi_search($args)
{
    extract($args);
    ...
    $mp3jukeboxtable = & $xartable['mp3jukebox'];
    $where = '';
    $releases = array();
    $sql = "SELECT  xar_exid,
                    xar_name,
    ...
    */
    ...
    for (; !$result->EOF; $result->MoveNext()) {
        ...
        if (xarSecurityCheck('ReadMP3Jukebox', 0)) {
            $mp3jukebox[] = array('exid'   => $exid,
                                'name'   => $name,
                                'number' => $number);
        }
    }
