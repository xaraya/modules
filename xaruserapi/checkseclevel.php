...
function crispbb_userapi_checkseclevel($args)
{
    ...
    if (empty($fid) && isset($check['fid']) && !empty($check['fid']) && is_numeric($check['fid'])) {
        $fid = $check['fid'];
    }
    ...
    if (empty($catid) && !empty($check['category']['id']) && is_numeric($check['category']['id'])) {
        $catid = (int)$check['category']['id'];
    }
    ...
    */
    ...
    if (isset($check) && is_array($check)) {
        if (empty($check['fprivileges'][$userLevel])) {
            return false;
        }
