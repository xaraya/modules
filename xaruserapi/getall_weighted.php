...
*/
function xtasks_userapi_getall_weighted($args)
{
    extract($args);
    ...
    $sql = "SELECT a.taskid,
                   ...
                   a.hours_remaining,
                   MAX(c.eventdate),
                   (b.priority + b.importance + a.priority + a.importance) as taskweight
            FROM ($xtaskstable a, $xprojecttable b)
            LEFT JOIN $worklogtable c
            ON a.taskid = c.taskid";
            
    $whereclause = array();
            
    $whereclause[] = "b.projectid = a.projectid";
    
    if(!empty($projecttype)) {
        $whereclause[] = "b.projecttype = '".$projecttype."'";
    }
...
//    if($groupid > 0) $sql .= " AND $taskcolumn[groupid] = $groupid";
    ...
    switch($orderby) {
        case "task_name":
            $sql .= " ORDER BY statusid, a.date_end_actual, a.task_name, c.eventdate DESC";
            break;
        case "importance":
            $sql .= " ORDER BY statusid, a.date_end_actual, a.importance, a.task_name, c.eventdate DESC";
            break;
        case "priority":
            $sql .= " ORDER BY statusid, a.date_end_actual, a.priority, a.task_name, c.eventdate DESC";
            break;
        case "status":
            $sql .= " ORDER BY statusid, a.date_end_actual, a.status, a.task_name, c.eventdate DESC";
            break;
        default:
            if(isset($mymemberid)) {
                $sql .= " ORDER BY taskweight, statusid, a.date_end_actual, a.date_start_planned, a.date_end_planned, a.priority, a.task_name, c.eventdate DESC ";
            } elseif(isset($memberid)) {
                $sql .= " ORDER BY taskweight, statusid, a.date_end_actual, a.date_start_planned, a.date_end_planned, a.importance, a.task_name, c.eventdate DESC ";
            } else {//if($statusfilter == "Closed") {
                $sql .= " ORDER BY taskweight, statusid, a.date_end_actual, a.date_start_planned, a.date_end_planned, a.task_name, c.eventdate DESC ";
            }
    }
/*
