 ...
 * @param int $args['playlistid'] start with this item number (default 1)
 * @return mixed array of items, or false on failure
 * @throws BAD_PARAM, DATABASE_ERROR, NO_PERMISSION
 */
function mp3jukebox_userapi_getallplaylistsongs($args)
{
     ...
     */
    $query = "SELECT $mp3jukebox_songs_table.xar_songid,
                    $mp3jukebox_songs_table.xar_song_filesize,
                    $mp3jukebox_songs_table.xar_createdate,
                    $mp3jukebox_songs_table.xar_status,
                    $mp3jukebox_songs_table.xar_song_name,
                    $mp3jukebox_songs_table.xar_song_filename,
                    $mp3jukebox_songs_table.xar_artist_name,
                    $mp3jukebox_songs_table.xar_album_name,
                    $mp3jukebox_songs_table.xar_song_filetype,
                    $mp3jukebox_playlist_songs_table.xar_sequence
              FROM 
                    $mp3jukebox_songs_table
              LEFT JOIN
                    $mp3jukebox_playlist_songs_table on $mp3jukebox_songs_table.xar_songid = $mp3jukebox_playlist_songs_table.xar_songid
              WHERE 
                    $mp3jukebox_playlist_songs_table.xar_playlistid = $playlistid
              ORDER BY 
                    $mp3jukebox_playlist_songs_table.xar_sequence, $mp3jukebox_songs_table.xar_song_name";
    ...
    */
    ...
    for (; !$result->EOF; $result->MoveNext()) {
        list($songid, $song_filesize, $createdate, $status, $song_name, $song_filename, $artist_name, $album_name, $song_filetype, $sequence) = $result->fields;
        if (xarSecurityCheck('ViewMP3Jukebox', 0, 'Song', "$song_name:All:$songid")) {
            $items[] = array('songid' => $songid,
