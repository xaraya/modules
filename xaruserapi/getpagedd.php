...
function publications_userapi_getpagedd($args)
{
    extract($args);
    ...
    foreach ($pages as $key => $page) {
        ...
        $itemtypes[$page['ptid']][$page['id']] = $key;
    }
    ...
    foreach($itemtypes as $itemtype => $items) {
        ...
        if (!xarModIsHooked('dynamicdata', 'publications', $itemtype)) {
            continue;
        }

        $dd_data = xarMod::apiFunc(
            'dynamicdata', 'user', 'getitems',
            array('module' => 'publications', 'itemtype' => $itemtype, 'itemids' => array_keys($items))
        );
