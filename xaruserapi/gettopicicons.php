...
function crispbb_userapi_gettopicicons($args)
{
    ...
    if (!empty($iconfolder)) {
        if (!empty($shownone)) {
            $iconlist['none'] = ['id' => 'none', 'name' => xarML('None')];
        }
        $moduleicons = xarMod::apiFunc(
            'crispbb',
            'user',
            'browse_files',
            [
        ...
        $themeicons = xarMod::apiFunc(
            'crispbb',
            'user',
            'browse_files',
            ['basedir' => xarTpl::getThemeDir() . '/modules/crispbb/images/'.$iconfolder,
                'match_re' => '/(gif|png|jpg)$/', ]
        );
        if (!empty($moduleicons)) {
            foreach ($moduleicons as $modicon) {
                ...
                $iconlist[$modicon] = ['id' => $modicon, 'name' => $iconname, 'imagepath' => $imagepath];
            }
        }
        unset($moduleicons);
        if (!empty($themeicons)) {
            foreach ($themeicons as $thmicon) {
                ...
                $iconlist[$thmicon] = ['id' => $thmicon, 'name' => $iconname, 'imagepath' => $imagepath];
            }
        }
        unset($themeicons);
    }
