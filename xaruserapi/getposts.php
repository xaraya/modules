...
function crispbb_userapi_getposts($args)
{
    ...
    $postsfields = array('pid', 'tid', 'powner','ptime','pstatus','poststype','psettings', 'pdesc', 'ptext', 'phostname');
    $topicfields = array('fid', 'tstatus', 'towner', 'ttype', 'topicstype','ttitle', 'tsettings', 'firstpid');
    ...
    if (empty($groupby) || $groupby == 'replies') {
        $query .= ' GROUP BY ' . $poststable . '.xar_pid';
    } else {
        $query .= ' GROUP BY ' . $poststable . '.xar_tid';
    }
    if (!empty($orderby)) {
        $query .= ' ORDER BY ' . join(',', $orderby);
    }else {
        $query .= ' ORDER BY ' . $poststable . '.xar_ptime ASC';
    }
