 ...
 */
function mp3jukebox_userapi_addsongs($args)
{
    ...
    if (!isset($addsong) || !is_array($addsong)) {
        $invalid[] = 'addsong';
    }
    if (count($invalid) > 0) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
            join(', ', $invalid), 'user', 'addsongs', 'MP3Jukebox');
    ...
    foreach($addsong as $song) {
        ...
        $result = $dbconn->Execute($query, array($playlistid, (int) $song, 0));
