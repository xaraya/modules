...
function uploads_userapi_get_file_list( $args )
{

    $path       = NULL;
    $files      = array();
    $sortdir    = _UPLOADS_VDIR_SORT_ASC;
    ...
    if (FALSE !== $vdirInfo) {
        ...
        if (in_array($vdirInfo['dirId'], array_keys($mountpoints))) {
           $files = xarModAPIFunc('uploads', 'fs', 'get_dir_contents', 
                                   array('path'     => $path, 
                                         'vdir_id'  => $vdirInfo['dirId'],
                                         'linkInfo' => $linkInfo));
        } else {
            $files = xarModAPIFunc('uploads', 'vdir', 'get_dir_contents', 
    ...
    foreach ($files as $entry => $data) {
        ...
        switch($sortby) {
            case _UPLOADS_VDIR_SORTBY_DATE:
