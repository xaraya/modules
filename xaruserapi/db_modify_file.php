...
function uploads_userapi_db_modify_file( $args )
{
    ...
    if (isset($fileName)) {
        ...
        $update_args[] = (string) $fileName;
    }


    if (isset($fileLocation)) {
        $update_fields[] = "xar_location=?";
        $update_args[] = (string) $fileLocation;
    }

    if (isset($userId)) {
        $update_fields[] = "xar_user_id = ?";
        $update_args[] = (int) $userId;
    }

    if (isset($fileStatus)) {
        $update_fields[] = "xar_status = ?";
        $update_args[] = (int) $fileStatus;
    }

    if (isset($store_type)) {
        $update_fields[] = "xar_store_type = ?";
        $update_args[] = (int) $store_type;
    }

    if (isset($fileType)) {
        $update_fields[] = "xar_mime_type = ?";
        $update_args[] = (string) $fileType;
    }
