...
function release_userapi_createid($args)
{
    ...
    if ((!isset($regname)) ||
        (!isset($exttype))) {
        throw new BadParameterException(null, xarML('Wrong arguments to release_userapi_createid'));
    }

    if (!isset($ridno)) {
        $ridno = 0;
    } //make it zero and check for this before using it for allocations
    $regname = trim(strtolower($regname));
    //get our new registration ID for this extension type
    $regid = xarMod::apiFunc(
        'release',
        'user',
        'allocateid',
