...
/**
 * Check to see if the current secondary ID
 * are listed in the intersections list
 *
 * @author Carl P. Corliss
 * @copyright 2004 (c) The Charles and Helen Schwab Foundation
 */

function navigator_userapi_count_articles_byseccat($args)
{
    extract ($args);
        ...
        $query = "SELECT pri.xar_cid as primary_cid,
               ...
               LEFT JOIN $catLinkTable as sec
                   ...
                   WHERE $artTable.xar_status IN (3, 2)
                         ...
                         AND pri.xar_modid = ?
                         AND pri.xar_cid = ?
                         AND sec.xar_cid IN ($primary_list)
               GROUP BY  pri.xar_cid, sec.xar_cid";

        $query_args = array($now, $articlesId, $secondary['id']);
        ...
        $query = "SELECT catlink.xar_cid as primary_cid,
               ...
               LEFT JOIN $catLinkTable AS catlink
                   ...
                   WHERE art.xar_status IN (3, 2)
                         ...
                         AND catlink.xar_cid IN ($secondary_list)
