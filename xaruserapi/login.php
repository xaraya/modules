 ...
 */
function authentication_userapi_login($args)
{
    extract($args);

    // FIXME: this should be removed as far as possible
    if (isset($passwd) && !isset($pass)) {
        die("authentication_userapi_login: authentication_userapi_login prototype has changed, " .
...
if ((!isset($uname)) ||
        (!isset($pass))) {
        $msg = xarML('Wrong arguments to authentication_userapi_login.');
