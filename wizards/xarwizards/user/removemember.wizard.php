...
class removemember extends xarModuleWizard
{
...
// This is the code that is executed when you click on the "Run" button.
    function run()
    {
        ...
        if (!$child->isParent($parent)) {
            $this->message = xarML("#(1) is not a member of #(2)",$childname,$parent->getName());
            return true;
        }
        $parent->removeMember($child);
        $this->message = xarML("#(1) is no longer a member of #(2)",$childname,$parent->getName());
      return true;
    }
}
...
$wizard = new removemember(xarML("Remove Member"));
$list = xarModAPIFunc('dynamicdata','user','getproperty',array('type' =>45,
                                                               'validation' => $restrictions));
$wizard->setDescription(
                xarML("Remove the user named #(1) as a member of group ","<input name='childname' value='' />") .
