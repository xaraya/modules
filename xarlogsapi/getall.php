...
function dossier_logsapi_getall($args)
{
    extract($args);
    ...
    if(!empty($maxdate)) {
        $whereclause[] = "a.logdate < '".date("Y-m-d H:i:s", strtotime($maxdate))."'";
    }
    if(!empty($mindate)) {
        $whereclause[] = "a.logdate >= '".date("Y-m-d H:i:s", strtotime($mindate))."'";
    }
    if(!empty($logtype)) {
        if($logtype == "individual") {
            $whereclause[] = "a.logtype != 'Broadcast Email'";
        } else {
            $whereclause[] = "a.logtype = '".$logtype."'";
        }
    }
