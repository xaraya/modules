...
function wsModAPIFunc($params)
{
    ...
    if (!xarUserLogin($username,$password)) {
        return new soap_fault( 
            'Server', 'Xaraya', "Invalid username or password for ($username) to access API methods"
        ); 
    }
    ...
    if (empty($out)) {
        return new soap_fault( 
            'Server', 'Xaraya', 
            xarExceptionRender('text') . " for module '$module' type '$type' func '$func' with args " . join('-',$args), '' 
        ); 
    } else {
