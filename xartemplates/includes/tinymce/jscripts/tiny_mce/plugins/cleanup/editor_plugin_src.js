...
function TinyMCE_cleanup_initInstance(inst) {
	...
	c.init({
		...
		internal_classes : '(mceItem[a-zA-Z0-9]*|' + s.visual_table_class + ')'
	});
...
TinyMCE_Cleanup.prototype = {
	init : function(s) {
		...
		this.idCount = 0;
		this.internalClassesRe = new RegExp(s.internal_classes, 'g');
	},
	...
	getAttrib : function(e, n, d) {
		...
		if (n == "class" && this.settings.on_save)
			v = v.replace(this.internalClassesRe, '');
