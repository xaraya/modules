function insertTable() {
	...
	args["rowtype"] = document.forms[0].rowtype.options[document.forms[0].rowtype.selectedIndex].value;

	tinyMCEPopup.execCommand("mceTableRowProps", false, args);
	tinyMCEPopup.close();
}
...
function init() {
	...
	for (var i=0; i<document.forms[0].rowtype.options.length; i++) {
		if (document.forms[0].rowtype.options[i].value == tinyMCE.getWindowArg('rowtype'))
			document.forms[0].rowtype.options.selectedIndex = i;
	}
