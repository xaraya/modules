...
// choose the best available format in the order xarg,gif,jpg
...
class Graph
{
    ...
    var $background_image="",$background_image_type=-1,$background_image_format="xarg";
...
// PRIVATE METHODS
    ...
    function LoadBkgImage($aImgFormat="xarg",$aBright=0,$aContr=0)
    {
    if( $aImgFormat == "jpg" )
...
class Image
{
    ...
    function StrokeBoxedText($x,$y,$txt,$dir=0,$fcolor="white",$bcolor="black", $shadowcolor=false,$paragraph_align="left", $xmarg=6,$ymarg=2,$cornerradius=0,$dropwidth=3)
    {
...
DEFINE(\"USE_APPROX_COLORS\",true);
    ...
    elseif( $y1==$y2 ) {        // Special case for horizontal lines
        ...
        $xarts = array($x2+$dx,$y2+$dy,$x2-$dx,$y2-$dy,$x1-$dx,$y1-$dy,$x1+$dx,$y1+$dy);
        imagefilledpolygon($this->img,$xarts,count($xarts)/2,$this->current_color);
    }
    ...
    if( $aFormat=="auto" ) {
        ...
        if( $aFormat=="jpeg" || $aFormat=="xarg" || $aFormat=="gif" ) {
        if( $aFormat=="jpeg" && !($supported & IMG_JPG) )
            $tst=false;
        elseif( $aFormat=="xarg" && !($supported & IMG_XARG) )
            $tst=false;
...
class RotImage extends Image
{
    ...
    function ArrRotate($xarts)
    {
    if( $this->a != 0 ) {
        for($i=0; $i < count($xarts)-1; $i+=2)
        list($xarts[$i],$xarts[$i+1]) = $this->Rotate($xarts[$i],$xarts[$i+1]);
    }
    return $xarts;
    }
    ...
    function Polygon($xarts)
    {
    //Polygon uses Line() so it will be rotated through that call
    parent::Polygon($xarts);
    }

    function FilledPolygon($xarts)
    {
    parent::FilledPolygon($this->ArrRotate($xarts));
    }
