...
class WURFL_Xml_DeviceIteratorTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $capabilities = ["product_info" => "" ];
        $wurflFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . self::WURFL_FILE;
        $deviceIterator = new WURFL_Xml_DeviceIterator($wurflFile, $capabilities);

        $capabilitiesSelected = ["brand_name", "model_name", "unique", "ununiqueness_handler", "is_wireless_device", "device_claims_web_support", "has_pointing_device", "has_qwerty_keyboard", "can_skip_aligned_link_row", "uaprof", "uaprof2", "uaprof3", "device_os" ];
        foreach ($deviceIterator as $device) {
    ...
    {
        $capabilities = ["product_info" => "is_wireless_device, device_claims_web_support" ];
        $wurflFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . self::WURFL_FILE;
        $deviceIterator = new WURFL_Xml_DeviceIterator($wurflFile, $capabilities);

        foreach ($deviceIterator as $device) {
            $this->assertEquals(["is_wireless_device", "device_claims_web_support" ], array_keys($device->getCapabilities()));
        }
    }
