...
class phpthumb {
	...
	var $phpthumb_version = '1.7.0-200602150817';
				...
				if ($getimagesize = GetImageSize($this->sourceFilename)) {
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							$side = min($this->source_width, $this->source_height, max($this->w, $this->wp, $this->wl, $this->ws, $this->h, $this->hp, $this->hl, $this->hs));
							if ($getimagesize[0] > $getimagesize[1]) {
								$commandline .= ' -thumbnail x'.$side;
							} else {
								$commandline .= ' -thumbnail '.$side.'x';
							}
