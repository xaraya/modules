...
class phpthumb {
	...
	var $phpthumb_version = '1.6.3-200511261736';
				...
				if ($ApacheLookupURIarray = phpthumb_functions::ApacheLookupURIarray($filename)) {
					...
					if (@is_readable($AbsoluteFilename)) {
						...
						if ($this->phpThumbDebug) {
							$this->DebugMessage('phpthumb_functions::ApacheLookupURIarray() failed for "'.$filename.'". This has been known to fail on Apache2 - try using the absolute filename for the source image', __FILE__, __LINE__);
							return false;
						} else {
	...
	function ImageCreateFromFilename($filename, &$rawimagedata) {
		...
		if ($getimagesizeinfo = @GetImageSize($filename)) {
			...
			if (!$gd_image) {
				...
				if (!$rawimagedata) {
					$this->DebugMessage('Populating $rawimagedata and attempting ImageCreateFromStringReplacement()', __FILE__, __LINE__);
			...
			$this->DebugMessage('GetImageSize('.$filename.') failed', __FILE__, __LINE__);
