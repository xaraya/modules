	...
	function iBrowser_open() {		
		...
		if (elm != null) { // if element
			if (elm.nodeName.toLowerCase() == 'img') { // selected object is image				
				...
				var oCaptionElement = ib.getNextSiblingByName(oImageElement, 'p');
				ib.getImgParam(oImageElement, wArgs); // set image parameters				)
				if (oLinkElement) { // set popup parameters					
					var oCaptionElement = ib.getNextSiblingByName(oLinkElement, 'p');
				...
				var oCaptionElement = ib.getChildByName(elm, 'p');
	...
	iBrowser.prototype.setCaption = function (elm,chkCaption, caption, captionClass) {
		if (chkCaption == 1) { // set caption
			...
			if (elm.nextSibling && elm.nextSibling.className == 'caption') { // existing caption
				...
				ib.isMSIE ? ib.setAttrib(capDiv, 'className', captionClass, true) : ib.setAttrib(capDiv, 'class', captionClass, true);	// set class				
				ib.isMSIE ? ib.setAttrib(capText, 'className', 'caption', true) : ib.setAttrib(capText, 'class', 'caption', true);		// set caption text
