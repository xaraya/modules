    ...
    elseif ($category_depth == 'products' || $manufacturers_id) {
        ...
        if ($manufacturers_id) {
          if (isset($_GET['filter_id']) && xarModAPIFunc('commerce','user','not_null',array('arg' => $_GET['filter_id']))) {
            ...
            $listing_sql = "select p.product_model, pd.product_name, m.manufacturers_name, p.product_quantity, p.product_image, p.product_weight, pd.product_short_description, pd.product_description, p.product_id, p.manufacturers_id, p.product_price, p.product_discount_allowed, p.product_tax_class_id, IF(s.status, s.specials_new_product_price, NULL) as specials_new_product_price, IF(s.status, s.specials_new_product_price, p.product_price) as final_price from " . TABLE_PRODUCTS . " p, " . TABLE_product_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m, " . TABLE_product_TO_CATEGORIES . " p2c left join " . TABLE_SPECIALS . " s on p.product_id = s.product_id where p.product_status = '1' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . $_GET['manufacturers_id'] . "' and p.product_id = p2c.product_id and pd.product_id = p2c.product_id and pd.language_id = '" . $_SESSION['languages_id'] . "' and p2c.categories_id = '" . $_GET['filter_id'] . "'";
          } else {
            // We show them all
            $listing_sql = "select p.product_model, pd.product_name, m.manufacturers_name, p.product_quantity, p.product_image, p.product_weight, pd.product_short_description, pd.product_description, p.product_id, p.manufacturers_id, p.product_price, p.product_discount_allowed, p.product_tax_class_id, IF(s.status, s.specials_new_product_price, NULL) as specials_new_product_price, IF(s.status, s.specials_new_product_price, p.product_price) as final_price from " . TABLE_PRODUCTS . " p, " . TABLE_product_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m left join " . TABLE_SPECIALS . " s on p.product_id = s.product_id where p.product_status = '1' and pd.product_id = p.product_id and pd.language_id = '" . $_SESSION['languages_id'] . "' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . $_GET['manufacturers_id'] . "'";
          }
        } else {
          // show the products in a given categorie
          if (isset($_GET['filter_id']) && xarModAPIFunc('commerce','user','not_null',array('arg' => $_GET['filter_id']))) {
            // We are asked to show only specific catgeory
            $listing_sql = "select p.product_model, pd.product_name, m.manufacturers_name, p.product_quantity, p.product_image, p.product_weight, pd.product_short_description, pd.product_description, p.product_id, p.manufacturers_id, p.product_price, p.product_discount_allowed, p.product_tax_class_id, IF(s.status, s.specials_new_product_price, NULL) as specials_new_product_price, IF(s.status, s.specials_new_product_price, p.product_price) as final_price from " . TABLE_PRODUCTS . " p, " . TABLE_product_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m, " . TABLE_product_TO_CATEGORIES . " p2c left join " . TABLE_SPECIALS . " s on p.product_id = s.product_id where p.product_status = '1' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . $_GET['filter_id'] . "' and p.product_id = p2c.product_id and pd.product_id = p2c.product_id and pd.language_id = '" . $_SESSION['languages_id'] . "' and p2c.categories_id = '" . $current_category_id . "'";
          } else {
            // We show them all
            $listing_sql = "select p.product_model, pd.product_name, m.manufacturers_name, p.product_quantity, p.product_image, p.product_weight, pd.product_short_description, pd.product_description, p.product_id, p.manufacturers_id, p.product_price, p.product_discount_allowed, p.product_tax_class_id, IF(s.status, s.specials_new_product_price, NULL) as specials_new_product_price, IF(s.status, s.specials_new_product_price, p.product_price) as final_price from " . TABLE_product_DESCRIPTION . " pd, " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on p.manufacturers_id = m.manufacturers_id, " . TABLE_product_TO_CATEGORIES . " p2c left join " . TABLE_SPECIALS . " s on p.product_id = s.product_id where p.product_status = '1' and p.product_id = p2c.product_id and pd.product_id = p2c.product_id and pd.language_id = '" . $_SESSION['languages_id'] . "' and p2c.categories_id = '" . $current_category_id . "'";
          }
        }
        ...
        if (PRODUCT_LIST_FILTER > 0) {
          if (isset($_GET['manufacturers_id'])) {
            $filterlist_sql = "select distinct c.categories_id as id, cd.categories_name as name from " . TABLE_PRODUCTS . " p, " . TABLE_product_TO_CATEGORIES . " p2c, " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where p.product_status = '1' and p.product_id = p2c.product_id and p2c.categories_id = c.categories_id and p2c.categories_id = cd.categories_id and cd.language_id = '" . $_SESSION['languages_id'] . "' and p.manufacturers_id = '" . $_GET['manufacturers_id'] . "' order by cd.categories_name";
          } else {
            $filterlist_sql = "select distinct m.manufacturers_id as id, m.manufacturers_name as name from " . TABLE_PRODUCTS . " p, " . TABLE_product_TO_CATEGORIES . " p2c, " . TABLE_MANUFACTURERS . " m where p.product_status = '1' and p.manufacturers_id = m.manufacturers_id and p.product_id = p2c.product_id and p2c.categories_id = '" . $current_category_id . "' order by m.manufacturers_name";
          }
