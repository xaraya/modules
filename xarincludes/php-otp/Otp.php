...
class Otp {
	...
	public function initializeOtp($userInput, $seed, $sequenceCount, $algorithm) {
		...
		// Check values
		if($this->isValidSeed($seed) !== true) throw new Exception('A valid seed is required', 101);
		if($this->isValidSequence($sequenceCount) !== true || $sequenceCount < 1) throw new Exception('A sequence count > 0 is required', 102);
		if($this->isAvailableAlgorithm($algorithm) !== true) throw new Exception('Chosen hash algorithm cannot be used', 103);

		// Attempt to convert the OTP provided by user
		if($this->isWordsOtp($userInput)) $otp = $this->convertOtp($userInput, 'words', 'hex');
		elseif($this->isHexOtp($userInput)) $otp = $userInput;
		else throw new Exception('Could not recognize provided one-time password', 104);
	...
	public function reinitializeOtp($userInput, $newSeed, $oldSeeds, $sequenceCount, $algorithm) {
		...
		if(!is_array($oldSeeds)) throw new Exception('An array or a string is expected as the "old seeds" argument', 201);

		// Check that the new seed is valid and hasn't been used before
		if($this->isValidSeed($newSeed) !== true) throw new Exception('A valid seed is required', 202);
		if(in_array($newSeed, $oldSeeds)) throw new Exception('The supplied seed has been used before', 203);
	...
	public function generateOtp($passPhrase, $seed, $sequence, $algorithm) {
		...
		// Check values
		if($this->isValidPassPhrase($passPhrase) !== true) throw new Exception('A valid passphrase is required', 401);
		if($this->isValidSeed($seed) !== true) throw new Exception('A valid seed is required', 402);
		if($this->isValidSequence($sequence) !== true) throw new Exception('A valid sequence number is required', 403);
		if($this->isAvailableAlgorithm($algorithm) !== true) throw new Exception('A valid algorithm is required', 404);
	...
	public function generateOtpList($passPhrase, $seed = null, $excludedSeeds = array(), $sequenceCount = null, $algorithm = null) {
		$sequenceCount = intval($sequenceCount);
		if(is_string($excludedSeeds)) $excludedSeeds = array($excludedSeeds);
		if(!is_array($excludedSeeds)) throw new Exception('An array or a string is expected as the "old seeds" argument', 501);

		// Set default values if not provided
		if(is_null($seed)) $seed = $this->generateSeed(false, false, $excludedSeeds);
		if(is_null($sequenceCount)) $sequenceCount = $this->defaultSequenceCount;
		if(is_null($algorithm)) $algorithm = $this->defaultAlgorithm;

		// Check values
		if($this->isValidPassPhrase($passPhrase) !== true) throw new Exception('A valid passphrase is required', 502);
		if($this->isValidSeed($seed) !== true) throw new Exception('A valid seed is required', 503);
		if(in_array($seed, $excludedSeeds)) throw new Exception('The supplied seed has been used before', 504);
		if($this->isValidSequence($sequenceCount) !== true || $sequenceCount < 1) throw new Exception('A valid sequence number is required', 505);
		if($this->isAvailableAlgorithm($algorithm) !== true) throw new Exception('A valid algorithm is required', 506);
	...
	protected function binaryOtp($passString, $algorithm) {
		if(is_null($passString) || empty($passString)) throw new Exception('A non-empty string is expected as an input', 601);
		if($this->isAvailableAlgorithm($algorithm) !== true) throw new Exception('A valid hash algorithm is required', 602);
	...
	public function convertOtp($inputOtp, $fromType, $toType) {
		if(!is_string($inputOtp) || empty($inputOtp)) throw new Exception('A non-empty string is expected as an input', 701);
		if(!is_string($fromType)) throw new Exception('The input type of the OTP must be specified', 702);
		if(!is_string($toType)) throw new Exception('The output type of the OTP must be specified', 703);
		...
		switch($fromType) {
			...
			case 'hex':
			case 'hexadecimal':
				if($this->isHexOtp($inputOtp) !== true) throw new Exception('The supplied input OTP is not hexadecimal', 704);
			...
			case 'word':
			case 'words':
				if($this->isWordsOtp($inputOtp) !== true) throw new Exception('The supplied input OTP is not a string of words', 705);
				...
				foreach($words as $word) {
					if(!in_array($word, $this->dictionary)) throw new Exception('One of the words has not been found in the dictionary', 707);
				...
				try {
					if($this->validateBitString($bitString) !== true) throw new Exception('Failed to process a word-based OTP', 708);
				} catch(Exception $e) {
			...
			case 'bit':
			case 'bits':
				if($this->isBitOtp($inputOtp) !== true) throw new Exception('The supplied input OTP is not a string of bits', 711);
			...
			case 'dec':
			case 'decimal':
				if($this->isDecOtp($inputOtp) !== true) throw new Exception('The supplied input OTP is not decimal', 712);
				$numbers = explode(' ', $inputOtp);

				$bitString = '';
				foreach($numbers as $number) {
					if(!array_key_exists($number, $this->dictionary)) throw new Exception('One of the numbers has not been found in the dictionary', 713);
					$dec = decbin($number);
					$bitString .= str_pad($dec, 11, '0', STR_PAD_LEFT);
				}
				...
				// Test the string of bits
				try {
					if($this->validateBitString($bitString) !== true) throw new Exception('Failed to process a decimal OTP', 714);
				} catch(Exception $e) {
		...
		if($otp === false || empty($otp)) throw new Exception('An unknowk error occurred during conversion', 718);
	...
	protected function validateBitString($bitString) {
		if(!is_string($bitString)) throw new Exception('A string was expected as an input', 801);
		if(!preg_match('/^[01]{66}$/', $bitString)) throw new Exception('The input string must be 66 characters long', 802);
	...
	public function createChallenge($seed, $sequence, $algorithm) {
		// Check seed
		if($this->isValidSeed($seed) !== true) throw new Exception('A valid seed was expected', 901);

		// Check sequence number
		if($this->isValidSequence($sequence) !== true) throw new Exception('A valid sequence number was expected', 902);

		// Check algorithm
		if($this->isValidAlgorithm($algorithm) !== true) throw new Exception('An available algorithm was expected', 903);
	...
	public function parseChallenge($challenge) {
		if(!is_string($challenge)) throw new Exception('A string is expected as an OTP challenge', 1001);

		// Test the challenge
		if($this->isValidChallenge($challenge) !== true) throw new Exception('The supplied string is not a valid OTP challenge', 1002);
	...
	public function isHexOtp($hexOtp) {
		if(!is_string($hexOtp)) return false;
	...
	public function isWordsOtp($wordsOtp) {
		if(!is_string($wordsOtp) || !preg_match('/^\s*[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s*$/', $wordsOtp)) return false;
	...
	public function isValidChallenge($challenge) {
		if(is_string($challenge) && preg_match('/^\s*otp-(md4|md5|sha1)\s+[0-9]+\s+[0-9a-z]+\s+$/', $challenge)) return true;
		else return false;
	}
	...
	public function isValidSeed($seed) {
		if(is_string($seed) && preg_match('/^[0-9a-zA-Z]{1,16}$/', $seed)) return true;
		else return false;
	}
	...
	public function isValidSequence($sequence) {
		if(is_numeric($sequence) && (preg_match('/^0$/', $sequence) || preg_match('/^[1-9][0-9]*$/', $sequence))) return true;
		else return false;
	}
	...
	public function generateSeed($minLength = null, $maxLength = null, $excludedSeeds = array()) {
		...
		if(is_null($minLength)) $minLength = 1;
		if(is_null($maxLength)) $maxLength = 16;
		if(!is_array($excludedSeeds)) $excludedSeeds = array();

		// Non-numeric values cannot be accepted
		if(is_numeric($minLength)) $minLength = intval($minLength);
		else throw new Exception('A numeric value is expected as the minimum length', 1101);

		if(is_numeric($maxLength)) $maxLength = intval($maxLength);
		else throw new Exception('A numeric value is expected as the maximum length', 1102);

		// Check that length is correct
		if($minLength < 1 || $minLength > 16) throw new Exception('The minimum length cannot be less then 1 or more than 16', 1103);
		if($maxLength < 1 || $maxLength > 16) throw new Exception('The maximum length cannot be less then 1 or more than 16', 1104);
		if($minLength > $maxLength) throw new Exception('The minimum lenght cannot be more than the maximum length', 1105);
		...
		while($seed == '' || in_array($seed, $excludedSeeds)) {
			...
			if($x > 1000) throw new Exception('Could not create a seed that would not be in the excluded seeds', 1106);
		}
	...
	protected function calculateChecksum($bitString) {
		if(!is_string($bitString)) throw new Exception('A string was expected as the only parameter.', 1201);
	...
	public function reformatHexOtp($hexOtp, $letterCase = null, $format = null) {
		if($this->isHexOtp($hexOtp) !== true) throw new Exception('A hexadecimal string expected as an input', 1601);
	...
	public function reformatWordsOtp($wordsOtp) {
		if(!is_string($wordsOtp) || !preg_match('/^\s*[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s*$/', $wordsOtp)) throw new Exception('A string of words expected as an input', 1701);
	...
	public function setAlternativeDictionary($dictionary) {
		if($this->isValidDictionary($dictionary) === false) throw new Exception('An array of words meeting RFC requirements is expected', 1801);
