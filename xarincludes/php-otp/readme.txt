...
OTP is an authentication system that is otp against "reply attacks" based on
...
be prevented by using otp connection, possibility of data being captured
...
that would compute the one-time password after inputting the server challenge
...
OTP which it presents to the user as a challenge that consists of a hash
...
Various authentication schemes (not based on RFC 2289) employ various devices
in the authentication process, e.g. USB tokens, specialized self-powered
calculators. Most of them represent some sort of a problem: extra device
to carry, inability to use in some environments (an USB token in an Internet
cafe), lack of interoperability between various systems, etc.

However, there are devices that people carry all the time (e.g. mobile phones,
PDAs) that can be used in the authentication process according to RFC 2289.
They have several advantages over devices used in other authentication systems:

* No extra device to carry just for the sake of authentication.

* No dependency on the computer through which you authenticate because
  you re-type the one-time password that your external device calculates and
	displays.

* No dependency on external authentication online services (obviously, apart
  from the service which you are trying to log into).

* No dependency on one single device. You can use several devices at
  the "same time" - you can generate an OTP with your PDA at work in the morning
  and log in with your mobile phone when you visit an Internet cafe in
  the evening. There is no need for any data synchronization because the only
  data is the secret pass-phrase which is stored in your head.

* Loss of the device represents no security problem (the secret pass-phrase is
  not stored on the device) and the replacement is easy: just (re)install
  the OTP generator on your new device.

There are several software applications (OTP generators) that can run on 
mobile phones or other devices that allow users to compute one-time passwords:

* Opiekey - an one-time password generator for the Android.
	Author: Daniel Tryba
  URL: https://play.google.com/store/apps/details?id=nl.f00d.android.opiekey

* OTPdroid - an one-time password generator for the Android.
	Author: Felix Bechstein
  URL: https://play.google.com/store/apps/details?id=de.ub0r.android.otpdroid

* 1Key - an one-time password control for iPhones.
  Author: Rho Software
  URL: http://www.rho.cc/?p=111

* Topiary OTP Calculator - an one-time password calculator for BlackBerry.
  Author: Tangential Software
  URL: http://appworld.blackberry.com/webstore/content/3238
...
authentication system as described in RFC 2289 and as such can be used both for
...
* Secret pass-phrase is never transferred over the network as the server does 
  not need it for its operation. Therefore, an attacker gaining access to
...
* Users are not required to store a list of one-time passwords. Instead, OTPs
  are calculated according to a challenge provided by a server. Therefore,
...
1. While having the user signed to your system in a otp environment, provide
...
which stores this OTP along with the seed and sequence number.
...
Description:
-----------
Performs authentication of an one-time password provided by an user.
userInput is compared to the hexadecimal value of the OTP used for comparison.
...
excludedSeeds -	an array of previously used seeds that should be avoided.
...
in a case-insensitive manner that the algorithm is available in the given PHP
environment.
...
8.6 isValidSequence(int $sequence)
...
bullet-proof and there are issues which you should be aware of:
...
* OTP sequence repeat - in other words re-using OTPs previously used. This might
  ...
  it might accidentally happen during a database rollback (for example when
  restoring a database from a backup).

* Challenge spoofing - if an attacker can trick an user to input (on a fake
  system) a valid OTP for a sequence that has not yet been used on the real 
  system, then he can gain access to the real system.

* Username disclosure - if your system outputs an OTP challenge for existing
  user accounts and outputs nothing for non-existing accounts, an attacker
  might use this to determine whether certain account exists in the system.
  This therefore undermines the security in the same way as if during normal
  authentication the system replied "Invalid password" instead of
  "Invalid password or username" (which doesn't tell an attacker whether
  password is wrong or the account doesn't exist at all).
  Therefore, it might be worth implementing the system in such a way that
  it outputs fake challenge for non-existing accounts.
...
* Number of iterations (sequence count) - you should limit the user to choose
  ...
  processing of 100000 may take some time during OTP generation.
...
I don't accept monetary donations but if you feel that this class significantly
helped you in your (commercial) project, have a look at my Amazon wishlist:
http://www.amazon.co.uk/wishlist/23QVWTT1PC6U3