<?php
/**
 *  iCalendar vjournal Parser Routine
 *  Returns a valid iCalendar_Event Object
 */


function &parse_vjournal(&$content,&$ical)
{
    //$vjournal =& new iCalendar_Event;
    $vjournal =& $ical->vjournal[$ical->num_vjournal()-1];
    ...
    foreach($content as $line_number => $line) {
        ...
        if(preg_match('/^\s+/',$line) && isset($assign_to)) {
            if((bool)$single) {
                ...
                $vjournal->{$assign_to} .= ' '.trim($line);
            } elseif((bool)$multiple) {
                $c = count($vjournal->{$assign_to}) - 1;
                // this is a wrapped line, prepend a space to it
                $vjournal->{$assign_to}[$c] .= ' '.trim($line);
            }
            ...
            if((bool)$single) {
                //find_attributes($assign_to,$vjournal);
                $vjournal->{$assign_to} = preg_replace('/[\n\r]/','',trim($vjournal->{$assign_to}));
                $vjournal->{$assign_to} = preg_replace('/^[:;]/','',$vjournal->{$assign_to});
            } elseif((bool)$multiple) {
                $c = count($vjournal->{$assign_to}) - 1;
                //find_attributes($assign_to,$vjournal,$c);
                $vjournal->{$assign_to}[$c] = preg_replace('/[\n\r]/','',trim($vjournal->{$assign_to}[$c]));
                $vjournal->{$assign_to} = preg_replace('/^[:;]/','',$vjournal->{$assign_to});
            }
       ...
       /* can have multiple of these */
        ...
        foreach($single_properties as $property) {
            if(preg_match('/^'.$property.'(;|:)?(.*)$/',$line,$match)) {
                ...
                $vjournal->{$assign_to} = substr($line,strlen($property)+1);
