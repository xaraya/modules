...
$here = dirname(__FILE__);
  require_once $here."/_parse_propfind.php";
  require_once $here."/_parse_proppatch.php";
  require_once $here."/_parse_lockinfo.php";
...
class HTTP_WebDAV_Server 
{
    ...
    function ServeRequest() 
    {
        ...
        // set path
        xarVarFetch('path','str::',$this->path,$this->base,XARVAR_NOT_REQUIRED);
        //$this->path = $this->_urldecode(!empty($_SERVER["PATH_INFO"]) ? $_SERVER["PATH_INFO"] : "/");
        if (substr($this->path,-1) != "/") { 
            $this->path .= "/";
        }
        //xarLogMessage('WebDAV: we worked out the path to be: ' .$this->path);
        ...
        //xarLogMessage("WebDAV: wrapper $wrapper");
    ...
    */
    ...
    {
        //xarLogMessage('WebDAV: base http_OPTIONS handler');
    ...
    {
        //xarLogMessage('WebDAV: base http_PROPFIND handler');
        $options = Array();
        ...
        if (!$this->propfind($options, $files)) {
            //xarLogMessage('WebDAV: the user handler for PROPFIND returned false');
        ...
        //xarLogVariable('WebDAV: return from user handler', $files['files']);
        ...
        // ... and payload
        // Start buffering,
        // FIXME: For Xaraya we can make this more clear by using the templates, postponed now though
        ob_start();
