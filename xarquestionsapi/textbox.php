...
function surveys_questionsapi_textbox($args) {
    if (!empty($args)) {
        return new surveys_questionsapi_textbox($args);
    } else {
        return 'surveys_questionsapi_textbox';
    }
}
...
class surveys_questionsapi_textbox extends surveys_questionsapi_default
{
    // Various properties.
    var $validation = NULL;
    var $hint = NULL;
    ...
    function surveys_questionsapi_textbox(&$args) {
        ...
        if (isset($this->dbquestion['dd']['validation'])) {
            $this->validation = $this->dbquestion['dd']['validation'];
        }
        if (!empty($this->dbquestion['dd']['hint'])) {
            $this->hint = $this->dbquestion['dd']['hint'];
        }
    }
    ...
    function render($args) {
        ...
        if ($this->target == 'output') {
            ...
            $template_data = $this->default_render_params();
            $template_data['question_text'] = &$question_text;
            $template_data['value'] = (isset($this->response['value1']) ? $this->response['value1'] : '');
    ...
    function validate() {
        ...
        // Do user-defined validation, if specified.
        if (!empty($this->validation) && $this->response['status'] == 'COMPLETE' && $this->response['value1'] != '') {
            // TODO: Clear any errors in the stack first?
            if (!xarVarValidate($this->validation, $this->response['value1'])) {
                // Validation failed.
                $currenterror = xarCurrentError();
                $msg = $currenterror->msg;
                xarErrorHandled();

                $this->response['status'] = 'INVALID';
                $this->error = xarML('failed validation - #(1)', $msg);
            }
        }
    ...
    function submit() {
        ...
        if ($result = xarVarFetch($name, 'pre:trim:left:200:trim:passthru:str', $value, '', XARVAR_NOT_REQUIRED)) {
    ...
    function init_question_type() {
        ...
        $propertydefs = array(
            'validation' => array('label'=>'Validation', 'type'=>'textbox', 'default'=>'', 'validation'=>''),
            'hint' => array('label'=>'Format hint', 'type'=>'textbox', 'default'=>'', 'validation'=>'')
        );
