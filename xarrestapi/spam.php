<?php
function twitter_restapi_spam($args)
{
    extract($args);
    ...
    if (!empty($invalid)) {
        $response['error'] = xarML('Invalid #(1) for Twitter API #(2) method', join(', ', $invalid), 'spam');
        return $response;
    }
    */  
    $path = array();
    ...
    if (!empty($invalid)) {
        $response['error'] = xarML('Invalid #(1) for Twitter API #(2) method #(3)', join(', ', $invalid), 'spam', $method);
        return $response;
    }
