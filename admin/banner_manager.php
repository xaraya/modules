<?php
/* --------------------------------------------------------------
   $Id: banner_manager.php,v 1.1 2003/09/06 22:05:29 fanta2k Exp $
  ...
  if ($_GET['action']) {
    switch ($_GET['action']) {
      case 'setflag':
        ...
        xarRedirectResponse(xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $_GET['bID']));
        break;
      case 'insert':
      case 'update':
        ...
        if (empty($html_text)) {
          if (!$banners_image = new upload('banners_image', DIR_FS_CATALOG_IMAGES . $banners_image_target)) {
            $banner_error = true;
          }
        }

        if (!$banner_error) {
          $db_image_location = (xarModAPIFunc('commerce','user','not_null',array('arg' => $banners_image_local))) ? $banners_image_local : $banners_image_target . $banners_image->filename;
          $q->addfield('banners_title',$banners_title,
                                  $q->addfield('banners_url',$banners_url);
                                  $q->addfield('banners_image',$db_image_location);
                                  $q->addfield('banners_group',$banners_group);
                                  $q->addfield('banners_html_text',$html_text);
          ...
          if ($_POST['expires_date']) {
            ...
            new xenQuery("update " . TABLE_BANNERS . " set expires_date = '" . xtc_db_input($expires_date) . "', expires_impressions = null where banners_id = '" . $banners_id . "'");
          } elseif ($_POST['impressions']) {
            $impressions = xtc_db_prepare_input($_POST['impressions']);
            new xenQuery("update " . TABLE_BANNERS . " set expires_impressions = '" . xtc_db_input($impressions) . "', expires_date = null where banners_id = '" . $banners_id . "'");
          }
          ...
          if ($_POST['date_scheduled']) {
            ...
            new xenQuery("update " . TABLE_BANNERS . " set status = '0', date_scheduled = '" . xtc_db_input($date_scheduled) . "' where banners_id = '" . $banners_id . "'");
          }

          xarRedirectResponse(xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners_id));
        } else {
          $_GET['action'] = 'new';
        }
        break;
      ...
      if(!$q->run()) return;
        ...
        new xenQuery("delete from " . TABLE_BANNERS . " where banners_id = '" . xtc_db_input($banners_id) . "'");
        new xenQuery("delete from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . xtc_db_input($banners_id) . "'");
        ...
        xarRedirectResponse(xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page']));
        break;
    }
  }
...
<script language="javascript">
  ...
  var dateScheduled = new ctlSpiffyCalendarBox("dateScheduled", "new_banner", "date_scheduled","btnDate2","<?php echo $bInfo->date_scheduled; ?>",scBTNMODE_CUSTOMBLUE);
      ...
      <tr><?php echo xtc_draw_form('new_banner', FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update')
  <input type="hidden" name="banners_id" value="#$bID#">
        ...
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php echo commerce_userapi_draw_pull_down_menu('banners_group', $groups_array, $bInfo->banners_group) . TEXT_BANNERS_NEW_GROUP . '<br>' . xtc_draw_input_field('new_banners_group', '', '', ((sizeof($groups_array) > 0) ? false : true)); ?></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="main"><?php echo xtc_draw_file_field('banners_image') . ' ' . TEXT_BANNERS_IMAGE_LOCAL . '<br>' . DIR_FS_CATALOG_IMAGES . xtc_draw_input_field('banners_image_local', $bInfo->banners_image); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo xtc_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            ...
            <td class="main"><?php echo DIR_FS_CATALOG_IMAGES . xtc_draw_input_field('banners_image_target'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo xtc_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr>
            ...
            <td class="main"><script language="javascript">dateExpires.writeControl(); dateExpires.dateFormat="dd/MM/yyyy";</script><?php echo TEXT_BANNERS_OR_AT . '<br>' . xtc_draw_input_field('impressions', $bInfo->expires_impressions, 'maxlength="7" size="7"') . ' ' . TEXT_BANNERS_IMPRESSIONS; ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<?php
    ...
    while ($banners = $q->output()) {
      ...
      if ( (is_object($bInfo)) && ($banners['banners_id'] == $bInfo->banners_id) ) {
        echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . xarModURL('commerce','admin',(FILENAME_BANNER_STATISTICS, 'page=' . $_GET['page'] . '&bID=' . $bInfo->banners_id) . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="javascript:popupImageWindow(\'' . FILENAME_POPUP_IMAGE . '?banner=' . $banners['banners_id'] . '\')">' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_popup.gif'), 'View Banner') . '</a>&#160;' . $banners['banners_title']; ?></td>
...
<?php
      if ($banners['status'] == '1') {
        echo xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_status_green.gif'), 'Active', 10, 10) . '&#160;&#160;<a href="' . xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=0') . '">' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_status_red_light.gif'), 'Set Inactive', 10, 10) . '</a>';
      } else {
        echo '<a href="' . xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=1') . '">' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_status_green_light.gif'), 'Set Active', 10, 10) . '</a>&#160;&#160;' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_status_red.gif', 'Inactive', 10, 10);
      }
?></td>
                <td class="dataTableContent" align="right"><?php echo '<a href="' . xarModURL('commerce','admin',(FILENAME_BANNER_STATISTICS, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id']) . '">' . xtc_image(xarTplGetImage('icons/statistics.gif'), ICON_STATISTICS) . '</a>&#160;'; if ( (is_object($bInfo)) && ($banners['banners_id'] == $bInfo->banners_id) ) { echo xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_arrow_right.gif'), ''); } else { echo '<a href="' . xarModURL('commerce','admin',(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id']) . '">' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'icon_info.gif'), IMAGE_ICON_INFO) . '</a>'; } ?>&#160;</td>
              </tr>
<?php
    }
?>
    ...
    default:
      if (is_object($bInfo)) {
        ...
        if ( (function_exists('imagecreate')) && ($dir_ok) && ($banner_extension) ) {
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'graphs/banner_infobox-' . $banner_id . '.' . $banner_extension));
        } else {
          include(DIR_WS_FUNCTIONS . 'html_graphs.php');
          $contents[] = array('align' => 'center', 'text' => '<br>' . xtc_banner_graph_infoBox($bInfo->banners_id, '3'));
        }

        $contents[] = array('text' => xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'graph_hbar_blue.gif', 'Blue', '5', '5') . ' ' . TEXT_BANNERS_BANNER_VIEWS . '<br>' . xtc_image(xarTplGetImage(DIR_WS_IMAGES . 'graph_hbar_red.gif'), 'Red', '5', '5') . ' ' . TEXT_BANNERS_BANNER_CLICKS);

        if ($bInfo->date_scheduled) $contents[] = array('text' => '<br>' . sprintf(TEXT_BANNERS_SCHEDULED_AT_DATE, xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$bInfo->date_scheduled))));

        if ($bInfo->expires_date) {
          $contents[] = array('text' => '<br>' . sprintf(TEXT_BANNERS_EXPIRES_AT_DATE, xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$bInfo->expires_date))));
        } elseif ($bInfo->expires_impressions) {
          $contents[] = array('text' => '<br>' . sprintf(TEXT_BANNERS_EXPIRES_AT_IMPRESSIONS, $bInfo->expires_impressions));
        }

        if ($bInfo->date_status_change) $contents[] = array('text' => '<br>' . sprintf(TEXT_BANNERS_STATUS_CHANGE, xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$bInfo->date_status_change))));
      }
      break;
  }
