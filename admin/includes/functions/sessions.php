...
/* --------------------------------------------------------------
  ...
  if (STORE_SESSIONS == 'mysql') {
    ...
    function _sess_write($key, $val) {
      ...
      if ($total['total'] > 0) {
        return new xenQuery("update " . TABLE_SESSIONS . " set expiry = '" . $expiry . "', value = '" . $value . "' where sesskey = '" . $key . "'");
      } else {
        return new xenQuery("insert into " . TABLE_SESSIONS . " values ('" . $key . "', '" . $expiry . "', '" . $value . "')");
      }
    }

    function _sess_destroy($key) {
      return new xenQuery("delete from " . TABLE_SESSIONS . " where sesskey = '" . $key . "'");
    }

    function _sess_gc($maxlifetime) {
      new xenQuery("delete from " . TABLE_SESSIONS . " where expiry < '" . time() . "'");
