...
/* --------------------------------------------------------------
  ...
  function xtc_banner_graph_monthly($banner_id) {
    ...
    $banner_stats_query = new xenQuery("select month(banners_history_date) as banner_month, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and year(banners_history_date) = '" . $year . "' group by month(banners_history_date)");
    while ($banner_stats = $q->output()($banner_stats_query)) {
