 ...
 * @return int jpgraph item ID on success, false on failure
 * @throws BAD_PARAM, NO_PERMISSION, DATABASE_ERROR
 */
function jpgraph_adminapi_create($args)
{
    ...
    if (count($invalid) > 0) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
            join(', ', $invalid), 'admin', 'create', 'JpGraph');
     ...
     */
    if (!xarSecurityCheck('AddJpGraph', 1, 'Item', "$name:All:All")) {
        return;
    }
     ...
     * modules
     */
    $jpgraphtable = $xartable['jpgraph'];
    /* Get next ID in table - this is required prior to any insert that
     * uses a unique ID, and ensures that the ID generation is carried
     * out in a database-portable fashion
     */
    $nextId = $dbconn->GenId($jpgraphtable);
     ...
     */
    $query = "INSERT INTO $jpgraphtable (
              xar_exid,
              xar_name,
              xar_number)
            VALUES (?,?,?)";
     ...
     */
    $exid = $dbconn->PO_Insert_ID($jpgraphtable, 'xar_exid');
