 ...
 */
function mp3jukebox_adminapi_create($args)
{
     ...
     */
    if (!xarSecurityCheck('AddMP3Jukebox', 1, 'Song', "$song_name:All:All")) {
        return;
    }
     ...
     */
    $nextId = $dbconn->GenId($mp3jukebox_songs_table);
     ...
     */
    $query = "INSERT INTO $mp3jukebox_songs_table (
              xar_songid,
              xar_createdate,
              xar_song_name,
              xar_artist_name,
     ...
     */
    $bindvars = array($nextId, time(), (string) $song_name, (string) $artist_name, (string) $album_name, (string) $status);
     ...
     */
    $songid = $dbconn->PO_Insert_ID($mp3jukebox_songs_table, 'xar_songid');
    ...
    $item['songid'] = $songid;
    xarModCallHooks('item', 'create', $songid, $item);
