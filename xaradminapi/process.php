...
function reminders_adminapi_process($args)
{
    ...
    $items = xarMod::apiFunc('reminders', 'user', 'getall', ['itemids' => $data['entry_list']]);
    ...
    */
    ...
    foreach ($items as $key => $row) {
        ...
        if ($row['due_date'] == $today) {
            // Send the email
            $data['result'] = xarMod::apiFunc('reminders', 'admin', 'send_email', ['info' => $row, 'params' => $params, 'copy_emails' => $args['copy_emails'], 'test' => $args['test']]);
        ...
        */
        ...
        $dates = xarMod::apiFunc('reminders', 'user', 'get_date_array', ['array' => $row]);
