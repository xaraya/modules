...
function addressbook_adminapi_updateprefixes($args)
{
    ...
    array_push($updates,array('sql' => "UPDATE $preTable
                                         SET name = ?
                                       WHERE nr = ?"
                             ,'bindvars' => array($modName[$k], $id)));
    }

    if(xarModAPIFunc('addressbook','admin','updateitems',array('tablename'=>'prefixes','updates'=>$updates))) {
    ...
    if(!empty($dels)) {
        ...
        if(xarModAPIFunc('addressbook','admin','deleteitems',array('tablename'=>'prefixes','delete'=>$delete))) {
        xarErrorSet(XAR_USER_EXCEPTION,
                    _AB_ERR_INFO,
    ...
    if( (isset($newname)) && ($newname != '') ) {
        if(xarModAPIFunc('addressbook','admin','additems',array('tablename'=>'prefixes','name'=>$newname))) {
        xarErrorSet(XAR_USER_EXCEPTION,
                    _AB_ERR_INFO,
