    ...
    */
    ...
    if ($args['test']) {
        ...
        foreach ($owners as $owner) {
            ...
            $row = xarMod::apiFunc('reminders', 'admin', 'generate_random_entry', ['user' => $owner]);

            // Rename the parameters for sending to the email template
            $row['lookup_id']    = $row['id'];
            ...
            // Send the email
            $data['result'] = xarMod::apiFunc('reminders', 'admin', 'send_email_lookup', ['owner' => $owner, 'info' => $row, 'params' => $params, 'copy_emails' => $args['copy_emails'], 'test' => $args['test']]);
            $data['results'] = array_merge($data['results'], [$data['result']]);
        }
    }
    return $data['results'];
