...
function bible_adminapi_scantextdir($args)
{
    ...
    if (!is_readable($textdir)) {
        xarSessionSetVar('statusmsg', xarML('Text directory does not exist or is not '
            . 'readable by web server.  Please set it before continuing.')
        );
    ...
    while (false !== ($file = readdir($hd))) {
        ...
        if (preg_match("/\.(vpl\.gz|dat\.gz|vpl|dat)\$/i", $file)) {
    ...
    foreach ($dbtexts as $text) {
        ...
        if (isset($newtexts[$text['file']])) unset($newtexts[$text['file']]);

        // generate name of config file we will look for
        $configfile = preg_replace("/\.(vpl\.gz|dat\.gz|vpl|dat)\$/i", '.conf', $filetext);

        // if text file changed, config just added, or config file changed...
        if ((file_exists($filetext) && $text['md5'] != $filetexts[$text['file']]) ||
            (file_exists($configfile) && $text['config_exists'] == false) ||
            (file_exists($configfile) && $text['md5_config'] != md5_file($configfile))
           ) {
        ...
        // or if text file or config file are missing...
        } elseif (
            ($text['state'] == 0 && !file_exists($filetext)) ||
            ($text['config_exists'] && !file_exists($configfile))
        ) {
    ...
    foreach ($newtexts as $file => $md5) {
        ...
        // get document type
        if (preg_match("/\.vpl(\.gz)?\$/i", $file)) {
            $type = 1;
        } elseif (preg_match("/\.dat(\.gz)?\$/", $file)) {
            $type = 2;
        } else {
            $type = 0;
        }
        ...
        if (file_exists($configfile)) {
            $config_exists = true;
            list($sname, $lname, $md5_config, $config) = xarModAPIFunc(
                'bible', 'user', 'parseconffile', array('file' => $configfile)
            );
        } else {
            $config_exists = false;
            $md5_config = '';
            $sname = strtoupper(preg_replace("/\.(vpl\.gz|dat\.gz|vpl|dat)\$/i", '', $file));
            $lname = $sname;
        }
