...
function reminders_adminapi_retire($args)
{
    ...
    if ($args['recurring'] == 1) {
        $entry = DataObjectMaster::getObject(['name' => 'reminders_entries']);
        $item = $entry->getItem(['itemid' => $args['itemid']]);
        $spawned = xarMod::apiFunc('reminders', 'admin', 'spawn', ['object' => $entry]);
        if (!$spawned) {
            return xarTpl::module('reminders', 'user', 'errors', ['layout' => 'not_spawned']);
        }
    }
