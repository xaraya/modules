...
function comments_adminapi_updateconfighook($args)
{
    ...
    xarModVars::set($modname, 'edittimelimit.' . $itemtype, $edittimelimit);
    xarModVars::set($modname, 'AllowPostAsAnon.' . $itemtype, $postanon);
    xarModVars::set($modname, 'AuthorizeComments.' . $itemtype, $authorize);
    xarModVars::set($modname, 'depth.' . $itemtype, $depth);
    xarModVars::set($modname, 'render.' . $itemtype, $render);
    xarModVars::set($modname, 'sortby.' . $itemtype, $sortby);
    xarModVars::set($modname, 'order.' . $itemtype, $order);
    xarModVars::set($modname, 'editstamp.' . $itemtype, $editstamp);
    xarModVars::set($modname, 'wrap.' . $itemtype, $wrap);
    xarModVars::set($modname, 'showoptions.' . $itemtype, $showoptions);
