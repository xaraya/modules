 ...
 */
function phplot_adminapi_getmenulinks()
{
    ...
    if (xarSecurityCheck('AddPHPlot', 0)) {

        $menulinks[] = array('url' => xarModURL('phplot','admin','new'),
    ...
    /* Security Check */
    if (xarSecurityCheck('EditPHPlot', 0)) {
         ...
         */
        $menulinks[] = array('url' => xarModURL('phplot','admin','view'),
            /* In order to display the tool tips and label in any language,
             * we must encapsulate the calls in the xarML in the API.
             */
            'title' => xarML('View phplot item, with options to modify and delete them.'),
            'label' => xarML('Manage Items'));
    }
    /* Security Check */
    if (xarSecurityCheck('AdminPHPlot', 0)) {
        /* We do the same for each new menu item that we want to add to our admin panels.
         * This creates the tree view for each item. Obviously, we don't need to add every
         * function, but we do need to have a way to navigate through the module.
         */
        $menulinks[] = array('url' => xarModURL('phplot','admin','modifyconfig'),
            /* In order to display the tool tips and label in any language,
             * we must encapsulate the calls in the xarML in the API.
             */
