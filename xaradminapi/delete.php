 ...
 * @param  $args ['songid'] ID of the item
 * @return bool true on success, false on failure
 * @throws BAD_PARAM, NO_PERMISSION, DATABASE_ERROR
 */
function mp3jukebox_adminapi_delete($args)
{
    ...
    if (!isset($songid) || !is_numeric($songid)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
            'song ID', 'admin', 'delete', 'MP3Jukebox');
     ...
     */
    if (!xarSecurityCheck('DeleteMP3Jukebox', 1, 'Song', "$item[song_name]:All:$songid")) {
        return;
    }
     ...
     */
    $query = "DELETE FROM $mp3jukebox_songs_table WHERE xar_songid = ?";

    /* The bind variable $songid is directly put in as a parameter. */
    ...
    $item['itemid'] = $songid;
    xarModCallHooks('item', 'delete', $songid, $item);
