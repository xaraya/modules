...
class Instance extends Base {
  ...
  */
  function sendTo($from,$activityId,$split=false) {
    ...
    if ($isInteractive=='n') {
$data = xarModAPIFunc('workflow','user','run_activity',
                      array('activityId' => $activityId,
                            'iid' => $iid,
                            'auto' => 1));
if (empty($data)) {
// TODO: clean this up
	trigger_error(tra("Fatal error: automatic activity produced an error"),E_USER_WARNING);      
	$dump = xarExceptionRender('html');
	echo var_dump($dump);
	die;
}
/*
