...
class ProcessManager extends BaseManager {
  ...
  */
  function serialize_process($pId)
  {
	...
	$fp=fopen(GALAXIA_DIR."/processes/$procname/code/shared.php","r");
	while(!feof($fp)) {
	  $line=fread($fp,8192);
	  $out.=$line;
	}
    ...
    while($res = $result->fetchRow(DB_FETCHMODE_ASSOC)) {    	
		...
		$out.='      </roles>'."\n";
		$out.='      <code><![CDATA[';
		$fp=fopen(GALAXIA_DIR."/processes/$procname/code/activities/$name.php","r");
		while(!feof($fp)) {
	  		$line=fread($fp,8192);
	  		$out.=$line;
		}
		...
		if($res['isInteractive']=='y') {
			$out.='      <template><![CDATA[';
			$fp=fopen(GALAXIA_DIR."/processes/$procname/code/templates/$name.tpl","r");
			while(!feof($fp)) {
		  		$line=fread($fp,8192);
		  		$out.=$line;
			}
	...
	foreach($transitions as $tran) {
	  ...
	  $out.='     </transition>'."\n";
   	...
   	$out.='  </transitions>'."\n";
    $out.= '</process>'."\n";
    //$fp = fopen(GALAXIA_DIR."/processes/$procname/$procname.xml","w");
    //fwrite($fp,$out);
    //fclose($fp);
    return $out;
  }
  ...
  */
  function import_process($data)
  {
	...
	$proc_info = $this->get_process($pid);
	$procname = $proc_info['normalized_name'];
	$fp = fopen(GALAXIA_DIR."/processes/$procname/code/shared.php","w");
	...
	$actids = Array();
      ...
      $fp = fopen(GALAXIA_DIR."/processes/$procname/code/activities/$actname".'.php',"w");
      fwrite($fp,$activity['code']);
      fclose($fp);
      if($activity['isInteractive']=='y') {
      	$fp = fopen(GALAXIA_DIR."/processes/$procname/code/templates/$actname".'.tpl',"w");
  ...
  function new_process_version($pId, $minor=true)
  {
	...
	$this->_rec_copy(GALAXIA_DIR."/processes/$oldname",GALAXIA_DIR."/processes/$newname");
    return $pid;
  }
  ...
  */
  function remove_process($pId)
  {
    ...
    $this->_remove_directory(GALAXIA_DIR."/processes/$name",true);
  ...
  */
  function replace_process($pId, $vars, $create = true)
  {
    ...
    if($pId) {
      ...
      if ($newname != $oldname) {
          rename(GALAXIA_DIR."/processes/$oldname",GALAXIA_DIR."/processes/$newname");
      }
   ...
   */
   function _create_directory_structure($name)
   {
     ...
     mkdir(GALAXIA_DIR."/processes/$name",0770);
	 mkdir(GALAXIA_DIR."/processes/$name/graph",0770);
	 mkdir(GALAXIA_DIR."/processes/$name/code",0770);
	 mkdir(GALAXIA_DIR."/processes/$name/compiled",0770);
	 mkdir(GALAXIA_DIR."/processes/$name/code/activities",0770);
     mkdir(GALAXIA_DIR."/processes/$name/code/templates",0770);
//     mkdir("templates/$name",0770);
	 // Create shared file
	 $fp = fopen(GALAXIA_DIR."/processes/$name/code/shared.php","w");
	 fwrite($fp,'<'.'?'.'php'."\n".'?'.'>');
	 fclose($fp);
   }
