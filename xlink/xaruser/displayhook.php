 ...
 */
function xlink_user_displayhook($args)
{
    extract($args);

    if (!isset($extrainfo)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'extrainfo', 'user', 'displayhook', 'xlink');
    ...
    if (!isset($objectid) || !is_numeric($objectid)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'object ID', 'user', 'displayhook', 'xlink');
        xarExceptionSet(XAR_USER_EXCEPTION, 'BAD_PARAM',
                       new SystemException($msg));
        return $msg;
    }
    ...
    if (empty($modid)) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'module name ' . $modname, 'user', 'displayhook', 'xlink');
        xarExceptionSet(XAR_USER_EXCEPTION, 'BAD_PARAM',
                       new SystemException($msg));
        return $msg;
    }
    ...
    if (empty($links) || !is_array($links) || count($links) == 0) return '';

    $data = array();

    foreach ($links as $link) {
       $link['url'] = xarModURL('xlink','user','main',
                                array('base' => $link['basename'],
                                      'id' => $link['refid']));
       $data['links'][] = $link;
    }
    return xarTplModule('xlink','user','displayhook',
                        $data);
}
