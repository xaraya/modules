...
/*
 * Group rule 'inlist'.
 * Validates a question list of values includes a specific value.
...
function surveys_rulesapi_inlist($args) {
    ...
    // Error in response (or no response yet).
    ...
    $value = explode(',', $response['value' . $params[2]]);
    //var_dump($value);

    // If the supplied value is in the array then the condition passes (true).
    if (in_array($params[1], $value)) {
        return true;
    } else {
        return false;
    }
