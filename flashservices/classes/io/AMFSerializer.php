...
class AMFSerializer
{
	...
	function writeArray($d)
	{
		if (is_array($d)) { // make damn certain the input is an array
			...
			$is_sequential = true;
			$check_key = 0; 
			//This now allows for non_sequential Arrays this will force the user 
			//to actually return a proper array, but ultimately serves the needs
			...
			//taken out to force a return of object. This should work for all arrays
			...
			} else if ($num_count > 0 && $is_sequential) { // this is just an array
				$this->out->writeByte(10); // write the mixed array code
				$this->out->writeLong($num_count); // write the count of items in the array
				foreach($numeric as $k=>$v) { 
					$this->writeData($numeric[$k]);
				}
			} else { // this is an object
				if($num_count > 0) { 
					$this->out->writeByte(3); // this is an object so write the object code
					$this->writeObject($numeric); // write the object name/value pairs
				} else { 
					$this->out->writeByte(3); // this is an object so write the object code
