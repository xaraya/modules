...
class AMFDeserializer
{
	...
	function readArray()
	{
		...
		for ($i=0; $i<$length; $i++) { // loop over all of the elements in the data
			...
			/****************************************************************************\
			 This was added 8-8-2004 to deal with arrays where an integer is used as an 
			 index into the array. i.e. if there's an index of 1298 this was sending an 
			 array from 0 - 1298 rather than just 1298
			\****************************************************************************/
			if(!empty($data)) { 
				$ret[$i] = $data;
			}
