 ...
 * @returns int
 * @return events item ID on success, false on failure
 * @raise BAD_PARAM, NO_PERMISSION, DATABASE_ERROR
 */
function events_adminapi_create($args)
{
    ...
    if (count($invalid) > 0) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    join(', ',$invalid), 'admin', 'create', 'Events');
    ...
    $nextId = $dbconn->GenId($eventstable);
    ...
    $exid = $dbconn->PO_Insert_ID($eventstable, 'xar_exid');
