...
class ievents_exportapi_handler_master
{
    ...
    // Fold lines that would be too long (for iCal and vCal formats)
    // TODO: we may need to escape each '\' with a further '\'
    // TODO: other characters need escaping, such as ','
    ...
    function old_fold_lines($string, $encoding = 'none', $limit = 76, $offset = 0)
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ((strlen($row) + $offset + strlen($enc)) > $fold) {
                ...
                if ($row[$lwsp] == ' ' || $row[$lwsp] == "\t") $delta = -1;

                $res[$res_ind] = substr($row, 0, $lwsp + 1 + $delta);

                // soft line break;
                if ($encoding == 'quotedprintable') $res[$res_ind] .= "=";

                $row = substr($row, $lwsp + 1);

                $row = ' ' . $row;

                if ($delta == -1 && $encoding == 'utf8') $row = ' ' . $row;

                //reduce row length of 1 to take into account the whitespace at the beginning of lines
                if ($res_ind == 0) $fold--;
            ...
            if ($string[$i] == ' ' || $string[$i] == "\t" || $string[$i] == ';' || $string[$i] == ',') $lwsp = strlen($row) - 1;

            // we cut at ':' only for quoted printable
            if ($string[$i] == ':' && $encoding == 'quotedprintable') $lwsp = strlen($row) - 1;
        } // end loop for each character

        // Add last row (or first if no folding is necessary)
        $res[$res_ind] = $row;
