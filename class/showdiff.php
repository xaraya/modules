...
class showdiff
{
	var $add_orig_array = array(),
		$add_final_array = array(),
		$delete_orig_array = array(),
		$delete_final_array = array(),
		$change_orig_array = array(),
		$change_final_array = array(),
		$copy_orig_array = array(),
		$copy_final_array = array(),
		$oldLineCheckFlag = array(),
		$newLineCheckFlag = array(),
		$highlight_old_arr = array(),
		$highlight_new_arr = array(),
		$orig_array = array(),
		$final_array = array(),
		$highlight_old ,
		$highlight_new ;

	function checkdiff($oldString , $newString , $diff,$Flag)
	{
...
*/
	...
	function highlightWords_orig($string_old , $changed , $deleted)
	{
	...
	function highlightWords_final($string_new, $changed , $added)
	{
	...
	function highlightLine_delete($string , $words)
	{
		$StringTemp = $string;
		foreach ( $words as $word )
		{
	...
	function highlightLine_added($string,$words)
	{
		$StringTempNew = $string;
		foreach ( $words as $word )
		{
	...
	function Line_copy_Old($string,$words)
	{
		foreach ( $words as $word )
		{
			foreach ($string as $key => $strItem)
	...
	function Line_copy_New($string,$words)
	{
		foreach ( $words as $word )
		{
			foreach ($string as $key => $strItem)
	...
	function highlightLines($string_old,$string_new)
	{
		$string_old = explode("\n",$string_old);
		$string_new = explode("\n",$string_new);
	...
	function highlightWords($string_old , $string_new)
	{
	...
	?><html>
			...
			<body>
				<table border="1">
					...
					<tr>
						<td>
							<?print_r("<pre>$this->highlight_old</pre>");?>
						</td>
						<td>
							<? print_r("<pre>$this->highlight_new</pre>");?>
						</td>
					</tr>
				</table>
			</body>
 ...
 function check_lastcharacter($str)
 {
