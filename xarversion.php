<?php
/**
    Owner - Tracks who creates xaraya based items.
 
    @package Xaraya Modules
    @copyright (C) 2003-2005 by Envision Net, Inc.
    @license GPL {@link http://www.gnu.org/licenses/gpl.html}
    @link http://www.envisionnet.net/
 
    @subpackage Owner module
	@link http://www.envisionnet.net/home/products/security/
    @author Brian McGilligan <brian@envisionnet.net>
*/

$modversion['name'] = 'Owner';
$modversion['id'] = '269';
$modversion['version'] = '0.6.1';
$modversion['description'] = ' A simple utility module that uses hooks to track which user created/owns a xaraya item.';
$modversion['credits'] = 'xardocs/credits.txt';
$modversion['help'] = 'xardocs/help.txt';
$modversion['changelog'] = 'xardocs/changelog.txt';
$modversion['license'] = 'xardocs/license.txt';
$modversion['official'] = 1;
$modversion['author'] = 'Brian McGilligan';
$modversion['contact'] = 'brian@envisionnet.net';
$modversion['admin'] = 1;
$modversion['user'] = 0;
$modversion['securityschema'] = array('owner::All' => '::');
$modversion['class'] = 'Complete';
$modversion['category'] = 'Global';
// this module depends on the categories module
$modversion['dependency'] = array();
?>