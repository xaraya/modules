 ...
 */
function contactform_admin_delete()
{
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from xarVarFetch(), getting them
    // from other places such as the environment is not allowed, as that makes
    // assumptions that will not hold in future versions of Xaraya
    if (!xarVarFetch('name',       'str:1',  $name,    'contactform',     XARVAR_NOT_REQUIRED)) return;
    ...
    if (empty($data['itemid'])) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    'item id', 'admin', 'delete', 'contactform');
    ...
    // do the check
    if (!xarSecurityCheck('DeleteContactForm',1,'Item',$data['itemid'])) return;
