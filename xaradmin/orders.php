...
require(DIR_WS_CLASSES . 'currencies.php');
  ...
  switch ($_GET['action']) {
    case 'update_order':
      ...
      $order_updated = false;
      $check_status_query = new xenQuery("select customers_name, customers_email_address, orders_status, date_purchased from " . TABLE_ORDERS . " where orders_id = '" . xtc_db_input($oID) . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
      $check_status = $q->output();
      if ($check_status['orders_status'] != $status || $comments != '') {
        new xenQuery("update " . TABLE_ORDERS . " set orders_status = '" . xtc_db_input($status) . "', last_modified = now() where orders_id = '" . xtc_db_input($oID) . "'");
      ...
      $data['NAME',$check_status['customers_name'];
      $data['ORDER_NR'] = $oID;
      $data['ORDER_LINK'] = xtc_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . $oID, 'SSL');
      $data['ORDER_DATE'] = xarModAPIFunc('commerce','user','date_long',array('raw_date' =>)$check_status['date_purchased']);
      $data['NOTIFY_COMMENTS'] = $comments;
      $data['ORDER_STATUS',] = $orders_status_array[$status];
        ...
        new xenQuery("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, date_added, customer_notified, comments) values ('" . xtc_db_input($oID) . "', '" . xtc_db_input($status) . "', now(), '" . $customer_notified . "', '" . xtc_db_input($comments)  . "')");
      ...
      xarRedirectResponse(xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('action')) . 'action=edit'));
      break;
    case 'deleteconfirm':
      $oID = xtc_db_prepare_input($_GET['oID']);
      ...
      xarRedirectResponse(xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('oID', 'action'))));
      break;
  }
...
<?php
    ...
    $banktransfer_query = new xenQuery("select banktransfer_prz, banktransfer_status, banktransfer_owner, banktransfer_number, banktransfer_bankname, banktransfer_blz, banktransfer_fax from banktransfer where orders_id = '" . xtc_db_input($_GET['oID']) . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
    $banktransfer = $q->output();
    ...
    for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
      ...
      if (sizeof($order->products[$i]['attributes']) > 0) {
        for ($j = 0, $k = sizeof($order->products[$i]['attributes']); $j < $k; $j++) {
         ...
         echo $currencies->format(xarModAPIFunc('commerce','user','add_tax',array('price' =>$order->products[$i]['attributes'][$j]['price'] * $order->products[$i]['qty'],'tax' =>$order->products[$i]['tax'])), true, $order->info['currency'], $order->info['currency_value']);
        } else {
...
<?php
    $orders_history_query = new xenQuery("select orders_status_id, date_added, customer_notified, comments from " . TABLE_ORDERS_STATUS_HISTORY . " where orders_id = '" . xtc_db_input($oID) . "' order by date_added");
    if ($orders_history_query->getrows()) {
      $q = new xenQuery();
      if(!$q->run()) return;
      while ($orders_history = $q->output()) {
        echo '          <tr>' . "\n" .
             '            <td class="smallText" align="center">' . xtc_datetime_short($orders_history['date_added']) . '</td>' . "\n" .
             '            <td class="smallText" align="center">';
        if ($orders_history['customer_notified'] == '1') {
          echo xtc_image(xarTplGetImage('icons/tick.gif', ICON_TICK) . "</td>\n";
        } else {
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><b><?php echo ENTRY_STATUS; ?></b> <?php echo commerce_userapi_draw_pull_down_menu('status', $orders_statuses, $order->info['orders_status']); ?></td>
              </tr>
              <tr>
            ...
            <td valign="top"><?php echo <input type="image" src="#xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_update.gif')#" border="0" alt=IMAGE_UPDATE>; ?></td>
          </tr>
        </table></td>
      </form></tr>
      <tr>
        <td colspan="2" align="right">
   <img src="<?php echo DIR_WS_CATALOG.'lang/'.$_SESSION['language'].'/admin/images/buttons/'; ?>button_invoice.gif" style="cursor:hand" onClick="window.open('<?php echo xarModURL('commerce','admin',(FILENAME_PRINT_ORDER,'oID='.$_GET['oID']); ?>', 'popup', 'toolbar=0, width=640, height=600')">
   <img src="<?php echo DIR_WS_CATALOG.'lang/'.$_SESSION['language'].'/admin/images/buttons/'; ?>button_packingslip.gif" style="cursor:hand" onClick="window.open('<?php echo xarModURL('commerce','admin',(FILENAME_PRINT_PACKINGSLIP,'oID='.$_GET['oID']); ?>', 'popup', 'toolbar=0, width=640, height=600')">
 <?php


 echo xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_back.gif'),'alt' => IMAGE_NEW_BACK); . '</a>'; ?></td>
      </tr>
<?php
  } else {
?>
  ...
  <tr>
    ...
    <td class="pageHeading" align="right">
              <?php echo xtc_draw_form('orders', FILENAME_ORDERS, '', 'get'); ?>
                <?php echo HEADING_TITLE_SEARCH . ' ' .
<input type="hidden" name="oID" value='size="12"'>
<input type="hidden" name="action" value='edit'>
  ...
  <tr>
    ...
    <td class="main" valign="top" align="right"><?php echo xtc_draw_form('status', FILENAME_ORDERS, '', 'get'); ?>
                <?php echo HEADING_TITLE_STATUS . ' ' . commerce_userapi_draw_pull_down_menu('status', xtc_array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_statuses), '', 'onChange="this.form.submit();"'); ?>
              </form></td>
  </tr>
</table>
...
<?php
    ...
    $orders_query = new xenQuery($orders_query_raw);
      $q = new xenQuery();
      if(!$q->run()) return;
    while ($orders = $q->output()) {
      ...
      if ( (is_object($oInfo)) && ($orders['orders_id'] == $oInfo->orders_id) ) {
        echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '\'">' . "\n";
      } else {
...
<?php
  ...
  switch ($_GET['action']) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . <input type="image" src="#xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_delete.gif')#" border="0" alt=IMAGE_DELETE> . ' <a href="' . xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id) . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_cancel.gif'),'alt' => IMAGE_CANCEL); . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>[' . $oInfo->orders_id . ']&#160;&#160;' . xtc_datetime_short($oInfo->date_purchased) . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_edit.gif'),'alt' => IMAGE_EDIT); . '</a> <a href="' . xarModURL('commerce','admin',(FILENAME_ORDERS, xtc_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=delete') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_delete.gif'),'alt' => IMAGE_DELETE); . '</a>');

        //$contents[] = array('align' => 'center', 'text' => '');

        $contents[] = array('text' => '<br>' . TEXT_DATE_ORDER_CREATED . ' ' . xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$oInfo->date_purchased)));
        if (xarModAPIFunc('commerce','user','not_null',array('arg' => $oInfo->last_modified))) $contents[] = array('text' => TEXT_DATE_ORDER_LAST_MODIFIED . ' ' . xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$oInfo->last_modified)));
