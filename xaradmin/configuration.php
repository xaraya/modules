...
//            include_once 'modules/products/includes/general.php';
            if (ereg('->', $use_function)) {
                $class_method = explode('->', $use_function);
                if (!is_object(${$class_method[0]})) {
                  include('modules/products/xarclasses/' . $class_method[0] . '.php');
                  ${$class_method[0]} = new $class_method[0]();
                }
                $cfgValue = xarModAPIFunc('products','admin','call_function',array(
        ...
        if (($cID == $configuration['configuration_id']) && (!isset($cInfo)) && (substr($action, 0, 3) != 'new')) {
            ...
            include 'modules/products/xarclasses/object_info.php';
//            include_once 'modules/products/inc/xtc_db_prepare_input.inc.php';
            $data['cInfo'] = $cInfo;
        }
    ...
    $data['gID'] = $gID;
    $data['action'] = $action;
    $data['thisurl'] = xarModURL('products','admin','configuration',array('gID'=>$gID));
