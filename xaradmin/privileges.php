...
//    if (!xarVarFetch('uid',          'isset', $uid,          NULL, XARVAR_DONT_SET)) {return;}
    ...
    if (!empty($extinstance)) {
        ...
        if (count($parts) > 0 && !empty($parts[0])) $personid = $parts[0];
        if (count($parts) > 1 && !empty($parts[1])) $cid = $parts[1];
        if (count($parts) > 2 && !empty($parts[2])) $uid = $parts[2];
        if (count($parts) > 3 && !empty($parts[3])) $persstatusid = $parts[3];
    }
/*
...
// TODO: figure out how to handle more than 1 category in instances
    ...
    if (empty($personid) || $personid == 'All' || !is_numeric($personid)) {
        $personid = 0;
    }
    $title = '';
    if (!empty($personid)) {
        $person = xarModAPIFunc('sigmapersonnel','user','get',
...
//    $data['pubtypes'] = xarModAPIFunc('articles','user','getpubtypes');
    $catlist = array();
    $cidstring = xarModGetVar('sigmapersonnel', 'mastercids');
        if (!empty($cidstring)) {
