 ...
 * @todo MichelV: everything
 */
function courses_admin_privileges($args)
{
    extract($args);
    ...
    if (empty($ptid) || $ptid == 'All' || !is_numeric($ptid)) {
        ...
        if (!xarSecurityCheck('AdminCourses',1,'Article',"$ptid:All:All:All")) return;
    }
...
// TODO: figure out how to handle more than 1 category in instances
    ...
    if (empty($courseid) || $courseid == 'All' || !is_numeric($courseid)) {
        $aid = 0;
    }
    $title = '';
    if (!empty($courseid)) {
        $course = xarModAPIFunc('courses','user','get',
