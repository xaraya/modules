    ...
    function payments_admin_modifyconfig()
    {
        ...
        switch (strtolower($phase)) {
            ...
            case 'update':
                ...
                switch ($data['tab']) {
                    case 'general':
                        if (!xarVar::fetch('itemsperpage', 'int', $itemsperpage, xarModVars::get('payments', 'itemsperpage'), xarVar::NOT_REQUIRED, xarVar::PREP_FOR_DISPLAY)) return;
                        if (!xarVar::fetch('shorturls', 'checkbox', $shorturls, false, xarVar::NOT_REQUIRED)) return;
                        if (!xarVar::fetch('modulealias', 'checkbox', $useModuleAlias,  xarModVars::get('payments', 'useModuleAlias'), xarVar::NOT_REQUIRED)) return;
                        if (!xarVar::fetch('aliasname', 'str', $aliasname,  xarModVars::get('payments', 'aliasname'), xarVar::NOT_REQUIRED)) return;
