 ...
 */
function labAccounting_admin_updateconfig()
{
/* we'll let our dynamic module settings be handled by DD here (optional)
    ...
    xarModSetVar('labAccounting', 'bold', $bold);

    if (!isset($itemsperpage) || !is_numeric($itemsperpage)) {
        $itemsperpage = 10;
    }
    xarModSetVar('labAccounting', 'itemsperpage', $itemsperpage);

    if (!isset($shorturls)) {
        $shorturls = 0;
    }
    xarModSetVar('labAccounting', 'SupportShortURLs', $shorturls);
*/

// TODO: fix xarResponseRedirect so that it doesn't exit anymore,
//       and move this below the update function itself again
    xarModCallHooks('module','updateconfig','labAccounting',
    ...
    xarResponseRedirect(xarModURL('labAccounting', 'admin', 'modifyconfig'));
