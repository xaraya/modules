...
/*/
 * validateitem
 * makes sure a new item is all good from the info recieved from additem
 *
 * @redirects you to viewitems, addpic, addreco, or additem
/*/
function shopping_admin_validateadditem()
{
    // security check
    ...
    if (!xarVarFetch('istock', 'str:1:', $istock, '', XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('gonext', 'int:1', $gonext, 0, XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('cids', 'str:1:', $cids, '', XARVAR_NOT_REQUIRED)) return;
    ...
    if ($errflag) {
      ...
      // call create item api func
      if (! $iid = xarModAPIFunc('shopping', 'admin', 'createitem',
                         array('iname' => $iname,
                               'iprice' => $iprice,
                               'isummary' => $isummary,
                               'idescription' => $idescription,
