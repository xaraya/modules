...
function commerce_admin_reviews()
{
  ...
  if ($_GET['action'] == 'edit') {
    ...
    $reviews_query = new xenQuery("select r.reviews_id, r.product_id, r.customers_name, r.date_added, r.last_modified, r.reviews_read, rd.reviews_text, r.reviews_rating from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd where r.reviews_id = '" . xtc_db_input($rID) . "' and r.reviews_id = rd.reviews_id");
      $q = new xenQuery();
      if(!$q->run()) return;
    $reviews = $q->output();
    $product_query = new xenQuery("select product_image from " . TABLE_PRODUCTS . " where product_id = '" . $reviews['product_id'] . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
    $products = $q->output();

    $product_name_query = new xenQuery("select product_name from " . TABLE_product_DESCRIPTION . " where product_id = '" . $reviews['product_id'] . "' and language_id = '" . $_SESSION['languages_id'] . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
    $product_name = $q->output();

    $rInfo_array = xtc_array_merge($reviews, $products, $product_name);
    $rInfo = new objectInfo($rInfo_array);
?>
      <tr><?php echo xtc_draw_form('review', FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $_GET['rID'] . '&action=preview'); ?>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="main" valign="top"><b><?php echo ENTRY_PRODUCT; ?></b> <?php echo $rInfo->product_name; ?><br><b><?php echo ENTRY_FROM; ?></b> <?php echo $rInfo->customers_name; ?><br><br><b><?php echo ENTRY_DATE; ?></b> <?php echo xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$rInfo->date_added)); ?></td>
            <td class="main" align="right" valign="top"><?php echo xtc_image(xarTplGetImage(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . $rInfo->product_image), $rInfo->product_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"'); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<input type="hidden" name="product_id" value="#$rInfo->product_id#">
<input type="hidden" name="customers_name" value="#$rInfo->customers_name#">
<input type="hidden" name="product_name" value="#$rInfo->product_name#">
<input type="hidden" name="product_image" value="#$rInfo->product_image#">
<input type="hidden" name="date_added" value="#$rInfo->date_added#">
    <input type="image" src="#xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_preview.gif')#" border="0" alt=IMAGE_PREVIEW>
        <a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $_GET['rID']) . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_cancel.gif'),'alt' => IMAGE_CANCEL); . '</a>'; ?></td>
      </form></tr>
<?php
    ...
    if ($_POST) {
      ...
      $reviews_query = new xenQuery("select r.reviews_id, r.product_id, r.customers_name, r.date_added, r.last_modified, r.reviews_read, rd.reviews_text, r.reviews_rating from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd where r.reviews_id = '" . $_GET['rID'] . "' and r.reviews_id = rd.reviews_id");
      $q = new xenQuery();
      if(!$q->run()) return;
      $reviews = $q->output();
      $product_query = new xenQuery("select product_image from " . TABLE_PRODUCTS . " where product_id = '" . $reviews['product_id'] . "'");
...
?>
      <tr><?php echo xtc_draw_form('update', FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $_GET['rID'] . '&action=update', 'post', 'enctype="multipart/form-data"'); ?>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="main" valign="top"><b><?php echo ENTRY_PRODUCT; ?></b> <?php echo $rInfo->product_name; ?><br><b><?php echo ENTRY_FROM; ?></b> <?php echo $rInfo->customers_name; ?><br><br><b><?php echo ENTRY_DATE; ?></b> #xarModAPIFunc('commerce','user','date_short',array('raw_date' =>$rInfo->date_added))#</td>
            <td class="main" align="right" valign="top"><?php echo xtc_image(xarTplGetImage(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . $rInfo->product_image), $rInfo->product_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"'); ?></td>
          </tr>
        </table>
      </tr>
...
?>
      <tr>
        <td align="right" class="smallText"><?php echo '<a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_back.gif'),'alt' => IMAGE_NEW_BACK); . '</a> ' . <input type="image" src="#xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_update.gif')#" border="0" alt=IMAGE_UPDATE> . ' <a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id) . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_cancel.gif'),'alt' => IMAGE_CANCEL); . '</a>'; ?></td>
      </form></tr>
<?php
    } else {
      if ($_GET['origin']) {
...
?>
      <tr>
        <td align="right"><?php echo '<a href="' . xarModURL('commerce','admin',($back_url, $back_url_params, 'NONSSL') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_back.gif'),'alt' => IMAGE_NEW_BACK); . '</a>'; ?></td>
      </tr>
<?php
    }
  } else {
...
<?php
    ...
    while ($reviews = $q->output()) {
      ...
      if(!$q->run()) return;
        ...
        $product_image_query = new xenQuery("select product_image from " . TABLE_PRODUCTS . " where product_id = '" . $reviews['product_id'] . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
      ...
      if(!$q->run()) return;
        ...
        $reviews_average_query = new xenQuery("select (avg(reviews_rating) / 5 * 100) as average_rating from " . TABLE_REVIEWS . " where product_id = '" . $reviews['product_id'] . "'");
      $q = new xenQuery();
      if(!$q->run()) return;
        $reviews_average = $q->output();

        $review_info = xtc_array_merge($reviews_text, $reviews_average, $product_name);
        $rInfo_array = xtc_array_merge($reviews, $review_info, $product_image);
        $rInfo = new objectInfo($rInfo_array);
      }
...
<?php
    ...
    switch ($_GET['action']) {
      case 'delete':
        ...
        $contents[] = array('text' => '<br><b>' . $rInfo->product_name . '</b>');
        $contents[] = array('align' => 'center', 'text' => '<br>' . <input type="image" src="#xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_delete.gif')#" border="0" alt=IMAGE_DELETE> . ' <a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id) . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_cancel.gif'),'alt' => IMAGE_CANCEL); . '</a>');
        break;

      default:
      if (is_object($rInfo)) {
        $heading[] = array('text' => '<b>' . $rInfo->product_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_edit.gif'),'alt' => IMAGE_EDIT); . '</a> <a href="' . xarModURL('commerce','admin','reviews', 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=delete') . '">' . xarModAPIFunc('commerce','user','image',array('src' => xarTplGetImage('buttons/' . xarSession::getVar('language') . '/'.'button_delete.gif'),'alt' => IMAGE_DELETE); . '</a>');
        ...
        $contents[] = array('text' => '<br>' . xtc_info_image($rInfo->product_image, $rInfo->product_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT));
        ...
        $contents[] = array('text' => '<br>' . TEXT_INFO_product_AVERAGE_RATING . ' ' . number_format($rInfo->average_rating, 2) . '%');
      }
        break;
    }
