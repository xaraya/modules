...
function comments_admin_modifyconfig()
{
    ...
    if (!xarVarFetch('tab', 'str:1:100', $data['tab'], 'comments_general', XARVAR_NOT_REQUIRED)) {
        return;
    }
    if (!xarVarFetch('tabmodule', 'str:1:100', $tabmodule, 'comments', XARVAR_NOT_REQUIRED)) {
        return;
    }
    ...
    switch (strtolower($phase)) {
        ...
        case 'update':
           ...
           // if (!xarSecConfirmAuthKey()) return;
            ...
            if (!xarVarFetch('editstamp', 'int', $editstamp, xarModVars::get('comments', 'editstamp'), XARVAR_NOT_REQUIRED)) {
                return;
            }

            if (!xarVarFetch('wrap', 'checkbox', $wrap, xarModVars::get('comments', 'wrap'), XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('numstats', 'str', $numstats, 20, XARVAR_NOT_REQUIRED)) {
                return;
            }

            if (!xarVarFetch('rssnumitems', 'int', $rssnumitems, xarModVars::get('comments', 'rssnumitems'), XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('showtitle', 'checkbox', $showtitle, xarModVars::get('comments', 'showtitle'), XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('enable_comments', 'checkbox', $showtitle, xarModVars::get('comments', 'enable_comments'), XARVAR_NOT_REQUIRED)) {
                return;
            }

        if (!xarVarFetch('filters_min_item_count', 'int', $filters_min_item_count, xarModVars::get('comments', 'filters_min_item_count'), XARVAR_NOT_REQUIRED)) {
            return;
        }
        if (!xarVarFetch('filters_min_item_count', 'int', $filters_min_item_count, xarModVars::get('comments', 'filters_min_item_count'), XARVAR_NOT_REQUIRED)) {
            return;
            ...
            if (!xarVarFetch('postanon', 'checkbox', $postanon, xarModVars::get('comments', 'postanon'), XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('useblacklist', 'checkbox', $useblacklist, xarModVars::get('comments', 'useblacklist'), XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('useblacklist', 'checkbox', $useblacklist, 1, XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('depth', 'str:1:', $depth, _COM_MAX_DEPTH, XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('render', 'str:1:', $render, _COM_VIEW_THREADED, XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('sortby', 'str:1:', $sortby, _COM_SORTBY_THREAD, XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('order', 'str:1:', $order, _COM_SORT_ASC, XARVAR_NOT_REQUIRED)) {
                return;
            }
           // if (!xarVarFetch('authorize', 'checkbox', $authorize, xarModVars::get('comments', 'authorize'), XARVAR_NOT_REQUIRED)) return;
            if (!xarVarFetch('authorize', 'checkbox', $authorize, 1, XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('usersetrendering', 'checkbox', $usersetrendering, xarModVars::get('comments', 'usersetrendering'), XARVAR_NOT_REQUIRED)) {
                return;
            }
