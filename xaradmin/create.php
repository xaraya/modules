...
function phplot_admin_create($args)
{
    ...
    if (count($invalid) > 0) {
         ...
         */
        return xarModFunc('phplot', 'admin', 'new',
                          array('name' => $name,
                                'number' => $number,
                                'invalid' => $invalid));
    }
     ...
     * In real modules, this needs to be *a lot smarter* than the phplot given here.
     *
     ...
     */
    $allItems = xarModApiFunc('phplot','user','getall');
    $newItemPosition = 0;
    foreach($allItems as $pos => $info) {
        if($info['exid']==$exid) {  $newItemPosition = $pos; break; }
    }
    xarResponseRedirect(xarModURL('phplot', 'admin', 'view',array('startnum' => $newItemPosition)));
