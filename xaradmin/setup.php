...
function otp_admin_setup($args)
{
    ...
    if (!xarVarFetch('algorithm', 'str', $algorithm, xarModVars::get('otp', 'algorithm'), XARVAR_DONT_SET)) return;
//    if (!xarVarFetch('seed',      'str', $seed,      '', XARVAR_DONT_SET)) return;
    if (!xarVarFetch('input',     'str', $input,     'BoDe Hop JaKe Stow juT RAP', XARVAR_DONT_SET)) return;
    if (!xarVarFetch('seq',       'str', $seq,       xarModVars::get('otp', 'sequence'), XARVAR_DONT_SET)) return;
