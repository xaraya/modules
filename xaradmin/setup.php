...
function otp_admin_setup($args)
{
    ...
    if (!xarVar::fetch('algorithm', 'str', $algorithm, xarModVars::get('otp', 'algorithm'), xarVar::DONT_SET)) return;
//    if (!xarVar::fetch('seed',      'str', $seed,      '', xarVar::DONT_SET)) return;
    if (!xarVar::fetch('input',     'str', $input,     'BoDe Hop JaKe Stow juT RAP', xarVar::DONT_SET)) return;
    if (!xarVar::fetch('seq',       'str', $seq,       xarModVars::get('otp', 'sequence'), xarVar::DONT_SET)) return;
