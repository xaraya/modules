...
function publications_admin_stylesheet_type($args)
{
    ...
    if (!xarVar::fetch('ptid', 'id', $data['ptid'], xarModVars::get('publications', 'defaultpubtype'), XARVAR_NOT_REQUIRED)) {
        return;
    }
    if (!xarVar::fetch('file', 'str', $data['file'], '', XARVAR_NOT_REQUIRED)) {
        return;
    }
