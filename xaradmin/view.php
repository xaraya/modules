...
function crispbb_admin_view($args)
{
    ...
    if (!empty($catid)) {
        $categories[$catid] = xarMod::apiFunc(
            'categories',
            'user',
            'getcatinfo',
            ['cid' => $catid]
    ...
    if (!empty($categories)) {
        foreach ($categories as $cid => $category) {
            ...
            $catinfo['forums'] = DataObjectMaster::getObjectList(['name' => 'crispbb_forums']);
            $catinfo['forums']->layout = 'admin';
            $fieldlist = ['fname', 'fdesc','fprivileges','ftype'];
            ...
            $catinfo['newforum'] = LinkCache::getCachedURL('crispbb', 'admin', 'new', ['catid' => $cid]);
            $catinfo['view'] = LinkCache::getCachedURL('crispbb', 'admin', 'view', ['catid' => $cid]);
            if ($userLevel == 800 && xarSecurity::check('ManageCategories', 0)) {
                if ($ci > 1) {
                    $catinfo['moveup'] = LinkCache::getCachedURL('crispbb', 'admin', 'ordercats', ['itemid' => $cid, 'direction' => 'up', 'authid' => $data['authid']]);
                }
                if ($ci < count($categories)) {
                    $catinfo['movedown'] = LinkCache::getCachedURL('crispbb', 'admin', 'ordercats', ['itemid' => $cid, 'direction' => 'down', 'authid' => $data['authid']]);
                }
                $catinfo['modifycat'] = LinkCache::getCachedURL('crispbb', 'admin', 'modifycat', ['itemid' => $cid]);
                $catinfo['deletecat'] = LinkCache::getCachedURL('crispbb', 'admin', 'deletecat', ['itemid' => $cid]);
            }
            $ci++;
