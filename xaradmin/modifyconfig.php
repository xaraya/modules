...
{
    // Security Check
    if (!xarSecurity::check('AdminSitemapper')) {
        return;
    }
        ...
        case 'update':
            ...
            switch ($data['tab']) {
                case 'general':
                    ...
                    if (!xarVar::fetch('file_create_xml', 'int', $file_create_xml, 0, xarVar::NOT_REQUIRED)) {
                        return;
                    }
                    if (!xarVar::fetch('file_create_zip', 'int', $file_create_zip, 0, xarVar::NOT_REQUIRED)) {
                        return;
                    }
                    if (!xarVar::fetch('file_xml_filename', 'str:1:', $xml_filename, 0, xarVar::NOT_REQUIRED)) {
                        return;
                    }
                    if (!xarVar::fetch('file_zip_filename', 'str:1:', $zip_filename, 0, xarVar::NOT_REQUIRED)) {
                        return;
                    }
