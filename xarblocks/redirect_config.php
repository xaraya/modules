...
class Wurfl_RedirectBlockConfig extends Wurfl_RedirectBlock
{
    ...
    public function update(array $data=array())
    {
        ...
        if (!xarVarFetch('redirects', 'array', $redirects, array(), XARVAR_NOT_REQUIRED)) {
            return;
        }
        $newredirects = array();
        ...
        if (!xarVarFetch('redirectua', 'pre:trim:str:1:', $redirectua, '', XARVAR_NOT_REQUIRED)) {
            return;
        }
        // only fetch other params if user agent isn't empty
        if (!empty($redirectua)) {
            if (!xarVarFetch('redirecttheme', 'pre:trim:str:1:', $redirecttheme, '', XARVAR_NOT_REQUIRED)) {
                return;
            }
            if (!xarVarFetch('redirecttemplate', 'pre:trim:str:1:', $redirecttemplate, '', XARVAR_NOT_REQUIRED)) {
                return;
            }
            $newredirects[] = array(
                'ua' => $redirectua,
