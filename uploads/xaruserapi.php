...
function uploads_userapi_transformhook ( $args )
{
	...
	$transformdata = $extrainfo['transform'];
	// Loop over each field that needs to be transformed
    for( $i=0; $i<sizeof($transformdata); $i++ )
    {
        $body = $extrainfo[$transformdata[$i]];
        ...
        $extrainfo[$transformdata[$i]] = $body;
    }
	return $extrainfo;	
}
?>