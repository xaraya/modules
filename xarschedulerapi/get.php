<?php
function dailydelicious_schedulerapi_get($args)
{
    extract($args);
    $GLOBALS['content'] = '';
    $GLOBALS['empty'] = true;
...
function startElement($parser, $name, $attrs) 
{
    if ($name == "POSTS") {
        if (strlen($GLOBALS['content']) == 0) { 
              ...
              $GLOBALS['content'] = $GLOBALS['content'] . "Shared bookmarks for <a href=\"http://del.icio.us/\">del.icio.us</a> user " . "<a href=\"http://del.icio.us/" . $attrs["USER"] . "\"> " . $attrs["USER"] . "</a><ul>\n";
        }
    } else {
        $GLOBALS['empty'] = false;
