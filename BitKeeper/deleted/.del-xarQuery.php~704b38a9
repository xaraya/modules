...
class xarQueryCollection
{
    ...
    {
        if (is_object($item)) $this->items[] = $item;
        elseif (is_string($item)) {}                    // TODO
        else {}                                         // TODO: error msg
    }
    ...
    {
        foreach ($items as $item) if ($item->getname() == $name) return $name;
        return;
    }
...
//---------------------------------------------------------
    function xarQuery($type='SELECT',$tables='',$fields='')
    {
        if (in_array($type,array("SELECT","INSERT","UPDATE","DELETE"))) $this->type = $type;
            ...
            $msg = xarML('The opreration #(1) is ot supported', $type);
    ...
    function row()
    {
        if (($this->result->fields) === false) $numfields = 0;
        else $numfields = $this->result->_numOfFields;
        for ($i=0;$i<$numfields;$i++) {
            $this->row[$this->fields[$i]['name']] = $this->result->fields[$i];
        }
        return $this->row;
    }
...
//            $this->fields = array_merge($this->fields,$fields);
    ...
    function eq($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => '=');
    }
    function gt($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => '>');
    }
    function ge($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => '>=');
    }
    function le($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => '<=');
    }
    function lt($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => '<');
    }
    function like($field1,$field2,$alias='')
    {
        $this->conditions[]=array('field1' => $field1,
                                  'field2' => $field2,
                                  'op' => 'like');
    ...
    function getconditions()
    {
        ...
        foreach ($this->conditions as $condition) {
            if (is_array($condition)) {
                $c .= $condition['field1'] . " " . $condition['op'] . " " . $condition['field2'] . " AND ";
            }
            else {
                // error msg
            }
            ...
            foreach ($this->fields as $field) {
                if (is_array($field)) {
                    if(isset($field['name']) && isset($field['value'])) {
                        $names .= $field['name'] . ", ";
                        $values .= "'" . xarVarPrepForStore($field['value']) . "', ";
                    }
                }
                else {
//                    $f .= $field . ", ";
        ...
        case "UPDATE" :
            ...
            foreach ($this->fields as $field) {
                if (is_array($field)) {
                    if(isset($field['name']) && isset($field['value']))
                    $f .= $field['name'] . " = '" . xarVarPrepForStore($field['value']) . "', ";
                }
                else {
//                    $f .= $field . ", ";
                }
