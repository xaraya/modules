...
function contact_adminapi_addCityCompany($args)
{
    ...
    if (count($invalid) > 0) {
        $msg = xarML('Invalid #(1) for #(2) function #(3)() in module #(4)',
                    join(', ',$invalid), 'admin', 'addCityCompany', 'contact');
    ...
    $nextId = $dbconn->GenId($contacttable);
     ...
     $dbconn =& xarDBGetConn();
    $xartable =& xarDBGetTables();
    $companytable = $xartable['contact_city'];
    $query = "SELECT count(*) FROM $companytable";
    ...
    $CompCity = $dbconn->PO_Insert_ID($contacttable, 'xar_id');
...
//    xarModCallHooks('item', 'create', $exid, 'exid');
    $item = $args;
    $item['module'] = 'contact';
    $item['itemid'] = $CompCity;
    xarModCallHooks('item', 'create', $CompCity, $item);
